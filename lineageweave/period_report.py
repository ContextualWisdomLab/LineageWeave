...
Leftover post–criterion pairs (ADR 0048 / 0181) come from the residual
...
distances on that map (Jeon et al., 2021, eq. 3). Complete-case
pairs also name leftover-map lengths ``‖ξ‖`` and ``‖ζ‖`` so
leftover-map magnitude is not read as leftover-map distance.
``fast-mlsirm`` has no leftover-pair API; this module does not invent
a second IRT fit and does not fork LSIRM.
