...
def _reader_evidence_kind(fact: str) -> str:
...
def _reader_evidence_text(fact: str) -> str:
...
def cited_post_evidence(
    ...
    not reader UI content. The evidence value itself remains visible so the
    ...
    for post_id in cited_post_ids:
        ...
        for fact in source.evidence_facts:
            text = _reader_evidence_text(fact)
            if not text or text in seen:
                continue
            seen.add(text)
            facts.append({"kind": _reader_evidence_kind(fact), "text": text})
