...
they are never treated as zero residuals. Each map pair names two-axis
leftover-map reconstruction ``R̂ = ξ_{1:2} · ζ_{1:2}`` so the
reconstructed leftover cell is not confused with leftover residual
``R`` or leftover-map distance ``d``.
"""
...
def leftover_pairs_from_residual(
    ...
    exist, two-axis reconstruction ``R̂ = ξ_{1:2} · ζ_{1:2}`` names the
    leftover cell the leftover map reconstructs. Fallback pairs (no
    complete-case map) omit reconstruction rather than fabricating one.
    Leftover-map distance stays full-rank Euclidean.
    """
...
def _candidate_row(
    ...
    """One observed leftover cell: distance, ids, residual, two-axis R̂."""
    return (
        max(distance, 0.0),
        post_ids[person],
        item_codes[item],
...
def _pair_from_candidate(
    ...
    return LeftoverPair(
        ...
        leftover_map_reconstruction=row[4],
    )
