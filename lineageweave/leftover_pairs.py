...
ADR 0164, ADR 0182, and ADR 0185.
...
leftover-map axes (ADR 0148). Complete-case coverage (ADR 0168) names
...
cell the map does not reconstruct is not confused with leftover
residual ``R`` or leftover-map distance ``d``. Each pair further names
leftover-map cross share ``x = 2 R̂ U / R²`` of the raw
...
Reconstruction ``R̂`` stays internal and is not persisted; only the ξ / ζ
coordinates are. ``x`` may be negative when reconstruction and unexplained
leftover have opposite signs. Without a complete-case rectangle, no pair or
coordinate is persisted; coverage counts explain why.
"""
...
class LeftoverInteractionMap:
    """Gabriel leftover-map points, axis shares, and the closest/farthest observed pairs."""
...
def leftover_pairs_from_residual(
    ...
    unexplained leftover ``U = R − R̂``. ``R̂`` stays internal and is
    never persisted. Without a complete-case map there is no pair
...
def leftover_map_from_residual(
    ...
    """Person, item, and axis-share leftover-map coordinates plus closest/farthest pairs.
    ...
    number of Gabriel singular values above the floor. Axis share is
    ``σ_k² / Σ_j σ_j²`` for leftover-map axes 1 and 2; rank-0 residuals
    emit two zero-share axes so seed can name leftover-map structure
    without inventing a leftover score.
    """
