...
Implements ADR 0048 as amended by ADR 0180.
...
map cosine so leftover-map alignment is not confused with leftover-map
distance ``d``.
"""
...
class LeftoverPair:
    ...
    leftover_map_cosine: float | None = None
...
def leftover_pairs_from_residual(
    ...
    have non-zero norms, leftover-map cosine names scale-free alignment.
    Origin or fallback pairs omit cosine rather than fabricating one.
    """
        ...
        for person, item in observed:
            ...
            candidates.append(
                _candidate_row(
                    ...
                    item,
                    distance,
                    _leftover_map_cosine(person_coord, item_coord),
                )
            )
    if not candidates:
        for person, item in observed:
...
def _leftover_map_cosine(person_coord: np.ndarray, item_coord: np.ndarray) -> float | None:
    """Scale-free leftover-map alignment; omit when either vector is origin."""
    person_norm = float(np.linalg.norm(person_coord))
    item_norm = float(np.linalg.norm(item_coord))
    if person_norm <= _LEFTOVER_SINGULAR_FLOOR or item_norm <= _LEFTOVER_SINGULAR_FLOOR:
        return None
    cosine = float(np.dot(person_coord, item_coord) / (person_norm * item_norm))
...
def _candidate_row(
    ...
    item: int,
    distance: float,
    leftover_map_cosine: float | None,
) -> tuple[float, str, str, float, float | None]:
    """One observed leftover cell: distance, ids, residual, map cosine."""
    return (
        max(distance, 0.0),
        post_ids[person],
        item_codes[item],
