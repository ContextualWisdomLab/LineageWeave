...
Implements ADR 0048 as amended by ADR 0181.
...
map lengths ``‖ξ‖`` and ``‖ζ‖`` so leftover-map magnitude is not
confused with leftover-map Euclidean distance ``d``.
"""
...
class LeftoverPair:
    ...
    leftover_map_person_length: float | None = None
    leftover_map_item_length: float | None = None
...
def leftover_pairs_from_residual(
    ...
    exist, leftover-map lengths name leftover-map magnitude independently
    of Euclidean distance. Fallback pairs (no complete-case map) omit
    lengths rather than fabricating them.
    """
    ...
    candidates: list[tuple[float, str, str, float, float | None, float | None]] = []
        ...
        for person, item in observed:
            ...
            candidates.append(
                _candidate_row(
                    ...
                    item,
                    distance,
                    _leftover_map_length(person_coord),
                    _leftover_map_length(item_coord),
                )
            )
    if not candidates:
        for person, item in observed:
...
def _leftover_map_length(coord: np.ndarray) -> float | None:
    """Gabriel leftover-map Euclidean length; omit non-finite norms."""
    value = float(np.linalg.norm(coord))
...
) -> tuple[float, str, str, float, float | None, float | None]:
    """One observed leftover cell: distance, ids, residual, map lengths."""
    return (
        max(distance, 0.0),
        post_ids[person],
        item_codes[item],
...
def _pair_from_candidate(
    pair_kind: str, row: tuple[float, str, str, float, float | None, float | None]
) -> LeftoverPair:
    ...
    return LeftoverPair(
        ...
        leftover_map_person_length=row[4],
        leftover_map_item_length=row[5],
    )
