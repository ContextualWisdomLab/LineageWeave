...
Implements ADR 0048 as amended by ADR 0183.
...
unexplained leftover share ``s = U_c² / R̃²`` of the *centered*
leftover the two-axis map factorizes, so that share is not confused
with leftover residual ``R``, leftover-map distance ``d``, or the
uncentered leftover ``U = R − ξ·ζ``. Reconstruction ``R̂_c`` and
centered leftover ``U_c`` stay internal and are not persisted.
"""
...
def leftover_pairs_from_residual(
    ...
    share rather than fabricating one. Leftover-map distance stays
    full-rank Euclidean.
    """
...
def _candidate_row(
    ...
    """One observed leftover cell: distance, ids, residual, unexplained share."""
    return (
        max(distance, 0.0),
        post_ids[person],
        item_codes[item],
...
def _pair_from_candidate(
    ...
    return LeftoverPair(
        ...
        leftover_map_unexplained_share=row[4],
    )
