...
rank so a rank-0 collapse is not read as leftover structure. Each pair
also names unexplained leftover ``U = R − R̂`` after two-axis Gabriel
reconstruction ``R̂ = ξ_{1:2} · ζ_{1:2}`` so the leftover cell the map
does not reconstruct is not confused with leftover residual ``R`` or
leftover-map distance ``d``. Reconstruction is computed internally and
is not persisted.
"""
