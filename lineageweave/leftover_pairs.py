...
residual ``R`` or leftover-map distance ``d``. Reconstruction ``R̂``
itself is computed internally and is not persisted; only the ξ / ζ
coordinates are. Without a complete-case rectangle, no pair is named
either -- coverage counts, not a center-distance stand-in pair, tell
the buyer why.
"""
