...
token is rejected. Its dev-only FastAPI `TestClient` uses Starlette's current
`httpx2` transport; `httpx` remains alongside it because FastAPI still declares
that compatibility dependency. `scripts/seed_demo_data.py` populates the docker-compose
