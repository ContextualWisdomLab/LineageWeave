...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f"{path}: expected exactly one repair marker, found {count}")
...
def add_red() -> None:
    ...
    if text.count(marker) != 1:
        raise SystemExit(f"current-parent App accessibility assertion marker count={text.count(marker)}")
...
''' + marker
    if helper_test_text.count(marker) != 1:
        raise SystemExit("cross-share edge-test marker changed")
