...
import {
  ...
  leftoverMapPlotCoordinatePairIsOrigin,
...
describe("leftoverMapPlotCoordinatePairIsOrigin", () => {
  it("names leftover-map origin independently of leftover-map item coordinates and leftover-map person coordinates", () => {
    expect(leftoverMapPlotCoordinatePairIsOrigin("(0.00, 0.00)")).toBe(true);
    expect(leftoverMapPlotCoordinatePairIsOrigin("(+0.50, \u22120.02)")).toBe(false);
    expect(leftoverMapPlotCoordinatePairIsOrigin("(+0.00, 0.00)")).toBe(false);
    expect(leftoverMapPlotCoordinatePairIsOrigin("(0.00, +0.00)")).toBe(false);
    ...
    expect(leftoverMapComparePlotPostBadge("Public post", 0, 0)?.key).toBe(
      LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN,
    );
    ...
    expect(leftoverMapListCriterionBadge("sales-lead", 0, 0)?.key).toBe(LEFTOVER_MAP_LIST_CRITERION_ORIGIN);
    ...
    expect(leftoverMapListCriterionBadge("sales-lead", 0, 0)?.key).not.toBe(LEFTOVER_MAP_LIST_CRITERION);
    expect(leftoverMapListCriterionBadge("sales-lead", 0, 0)?.key).not.toBe(
      LEFTOVER_MAP_LIST_POST_ACTION_ORIGIN,
    );
    ...
    expect(leftoverMapCompareListPostBadge("Public post", 0, 0)?.key).toBe(
      LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN,
    );
...
describe("leftoverMapListPostBadge", () => {
  ...
  it("stays distinct from leftover-map graphic leftover-map post markers and leftover-map comparison graphic leftover-map post markers", () => {
    ...
    expect(LEFTOVER_MAP_LIST_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_LIST_CRITERION_ORIGIN);
    expect(LEFTOVER_MAP_LIST_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN);
...
describe("leftoverMapListCriterionBadge", () => {
  ...
  it("names rank-0 leftover-map origin leftover-map item coordinates independently of leftover-map pair leftover-map post leftover-map origin leftover-map person coordinates", () => {
    expect(leftoverMapListCriterionBadge("sales-lead", 0, 0)).toEqual({
      key: LEFTOVER_MAP_LIST_CRITERION_ORIGIN,
  ...
  it("stays distinct from leftover-map graphic leftover-map criterion markers, leftover-map comparison graphic leftover-map criterion markers, and leftover-map pair leftover-map post leftover-map person coordinates", () => {
    ...
    expect(LEFTOVER_MAP_LIST_CRITERION_ORIGIN).toBe(
      "leftover pair leftover-map criterion {label} at leftover-map origin ζ {item}",
    );
    expect(LEFTOVER_MAP_LIST_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_LIST_CRITERION);
    expect(LEFTOVER_MAP_LIST_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN);
    expect(LEFTOVER_MAP_LIST_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN);
    expect(LEFTOVER_MAP_LIST_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_LIST_POST_ACTION_ORIGIN);
...
describe("leftoverMapCompareListPostBadge", () => {
  ...
  it("names rank-0 leftover-map origin leftover-map person coordinates independently of leftover-map pair leftover-map post leftover-map origin leftover-map person coordinates", () => {
    expect(leftoverMapCompareListPostBadge("Public post", 0, 0)).toEqual({
      key: LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN,
      values: { title: "Public post", person: "(0.00, 0.00)" },
    });
    expect(leftoverMapCompareListPostBadge("Public post", 0, 0)?.key).not.toBe(
      LEFTOVER_MAP_COMPARE_LIST_POST_ACTION,
  ...
  it("stays distinct from leftover-map pair leftover-map post leftover-map person coordinates, leftover-map graphic leftover-map post markers, and leftover-map comparison graphic leftover-map post markers", () => {
    ...
    expect(LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN).toBe(
      "leftover map comparison leftover pair leftover-map post {title} at leftover-map origin ξ {person}",
    );
    expect(LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_LIST_POST_ACTION);
    expect(LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_LIST_POST_ACTION_ORIGIN);
    expect(LEFTOVER_MAP_COMPARE_LIST_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN);
