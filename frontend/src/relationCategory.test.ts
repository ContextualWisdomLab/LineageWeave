...
describe("precedenceFromEdges", () => {
  ...
  it("orders skos_broader's target (the broader concept) ahead of its source (the narrower one)", () => {
    // Every skos:broader triple in the ontology has the narrower concept as
    // subject/source and the broader concept as object/target (e.g.
    // :CompanyLevel skos:broader :GroupLevel -- Company is narrower). This
    // must match the identical narrower-source/broader-target shape every
    // other hierarchical predicate in ORDER_DIRECTION uses.
    const precedes = precedenceFromEdges([
      { source: "company", target: "group", edge_type_code: "skos_broader" },
    ]);
    expect(precedes.get("group")).toEqual(["company"]);
  });
