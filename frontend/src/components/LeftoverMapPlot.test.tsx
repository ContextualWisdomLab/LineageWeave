...
describe("LeftoverMapPlot", () => {
  ...
  it("captions leftover-map axes with persisted leftover-map singular values and leftover-map axis share", () => {
    ...
    expect(screen.getByLabelText("leftover-map axis 2 tick −0.02 σ 0.86 18%")).toBeInTheDocument();
  ...
  it("captions leftover-map graphic leftover-map axis ticks with leftover-map axis share independently of leftover-map singular values", () => {
    render(
      <LeftoverMapPlot
        pairs={PAIRS}
        leftoverMapAxes={[
    ...
    expect(screen.getByLabelText("leftover-map axis 1 tick +0.50 82%")).toBeInTheDocument();
    expect(screen.getByLabelText("leftover-map axis 2 tick −0.02 18%")).toBeInTheDocument();
    expect(screen.queryByLabelText("leftover-map axis 1 tick +0.50 σ 1.84")).not.toBeInTheDocument();
    expect(screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 tick +0.50 82%")).not.toBeInTheDocument();
  });

  it("captions leftover-map graphic leftover-map axis ticks with leftover-map singular values and leftover-map axis share together", () => {
    render(
      <LeftoverMapPlot
        pairs={PAIRS}
        leftoverMapAxes={[
    ...
    expect(screen.getByLabelText("leftover-map axis 2 tick −0.02 18%")).toBeInTheDocument();
    expect(screen.queryByLabelText("leftover-map axis 1 tick +0.50 σ 1.84")).not.toBeInTheDocument();
    expect(screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 tick +0.50 σ 1.84 82%")).not.toBeInTheDocument();
  });
  ...
  it("omits leftover-map graphic-display singular values independently of leftover-map axis share", () => {
    render(
      <LeftoverMapPlot
        pairs={PAIRS}
        leftoverMapAxes={[
  ...
  it("omits leftover-map comparison graphic leftover-map rank when rank is missing or not usable", () => {
    render(
      <LeftoverMapPlot
        pairs={[
          { ...PAIRS[0], leftover_map_rank: null },
          { ...PAIRS[1], leftover_map_rank: 1.5 },
        ]}
        leftoverMapAxes={[
          { axis_index: 1, leftover_singular_value: 1.84, leftover_share: 0.82 },
          { axis_index: 2, leftover_singular_value: 0.86, leftover_share: 0.18 },
  ...
  it("omits leftover-map comparison graphic leftover-map distance when d is missing or not usable", () => {
    render(
      <LeftoverMapPlot
        pairs={[
          { ...PAIRS[0], leftover_distance: Number.NaN },
          { ...PAIRS[1], leftover_distance: Number.POSITIVE_INFINITY },
        ]}
        leftoverMapAxes={[
          { axis_index: 1, leftover_singular_value: 1.84, leftover_share: 0.82 },
          { axis_index: 2, leftover_singular_value: 0.86, leftover_share: 0.18 },
  ...
  it("omits invented leftover-map comparison graphic leftover-map axis ticks that no persisted coordinate occupies", () => {
    ...
    expect(
      screen.getAllByLabelText("leftover map comparison graphic leftover-map axis 1 origin tick 0.00 σ 0.00 0%").length,
    ).toBeGreaterThan(0);
    expect(
      screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 tick 0.00"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 origin tick 0.00"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 tick 0.00 σ 0.00"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 origin tick 0.00 σ 0.00"),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("names rank-0 origin leftover-map coordinate ticks on the comparison graphic", () => {
    ...
    expect(
      screen.getByLabelText("leftover map comparison graphic leftover-map axis 1 origin tick 0.00 σ 0.00 0%"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("leftover map comparison graphic leftover-map axis 2 origin tick 0.00 σ 0.00 0%"),
    ).toBeInTheDocument();
    expect(
      screen.queryByLabelText("leftover map comparison graphic leftover-map axis 1 tick 0.00 σ 0.00 0%"),
    ).not.toBeInTheDocument();
    expect(screen.queryByLabelText("leftover-map axis 1 tick 0.00")).not.toBeInTheDocument();
  ...
  it("captions leftover-map comparison graphic leftover-map axis ticks with leftover-map singular values independently of leftover-map axis share", () => {
    ...
    expect(
      screen.getByLabelText("leftover map comparison graphic leftover-map axis 1 origin tick 0.00 σ 1.84"),
    ).toBeInTheDocument();
    expect(
