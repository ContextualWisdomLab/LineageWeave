...
describe("OntologyExplorer", () => {
  ...
  it("keeps complete long node labels in the rendered graph and exact-value table", () => {
    const longLabel = "Synthetic multilingual procurement governance decision with complete provenance";
    const payload = neighborhood({
      nodes: neighborhood().nodes.map((node, index) => index === 0 ? { ...node, display_label: longLabel } : node),
      exact_value_rows: neighborhood().exact_value_rows.map((row, index) => index === 0 ? { ...row, source_label: longLabel } : row),
    });
    render(<OntologyExplorer focusNodeType="node_post" focusNodeId={POST_ID} neighborhood={payload} />);
