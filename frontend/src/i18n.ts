...
const TRANSLATIONS: Partial<Record<Locale, Record<string, string>>> = {
  ko: {
    ...
    Resolved: "종료 확인", Open: "진행 중", "Insufficient measurement evidence": "측정 근거 부족", "Event time": "사건 발생일", "Record creation time": "기록 생성일",
    "{start} ~ {end} · Event time": "{start} ~ {end} · 사건 발생일", "{start} or later · Event time": "{start} 이후 · 사건 발생일", "Through {end} · Event time": "{end} 이전 · 사건 발생일", "All time · Event time": "전체 기간 · 사건 발생일",
    ...
    "All posts": "전체 글", "Classified events": "분류 사건", "External information (share of all posts)": "외부 정보 (전체 글 대비)", "{count} records · {percent}%": "{count}건 · {percent}%", "Analysis pending": "분석 대기", "Analysis failed": "분석 실패",
    "Status by work type": "업무 유형별 현황", "{events} events · {posts} posts": "사건 {events}건 · 글 {posts}건", "Observed processing intervals": "관측된 처리 구간", "Compare elapsed time for items with observed start and end events.": "시작과 종료 사건이 확인된 항목의 경과 시간을 비교하세요.",
    "Open {open} · resolved {resolved} · insufficient measurement evidence {missing}": "진행 중 {open}건 · 종료 확인 {resolved}건 · 측정 근거 부족 {missing}건", "Observed events by project": "프로젝트별 관측 사건", "Analyzing project connection": "프로젝트 연결 분석 중", "Confirmed elapsed time": "확정 경과 시간", "Elapsed time is calculated when all required start and end event evidence is observed.": "경과 시간은 필요한 시작·종료 사건 근거가 모두 관측될 때 계산됩니다.",
