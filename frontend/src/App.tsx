...
import { AnalysisRunListButton } from "./components/AnalysisRunListButton";
import { AnalysisRunRequestButton } from "./components/AnalysisRunRequestButton";
...
function AnalysisRunsPanel({
  ...
  return (
    <section className="popup-section lineage-home">
      <div className="lineage-home-header">
        <h2>Analysis runs</h2>
        <AnalysisRunRequestButton
          requesting={requesting}
          onRequest={() => void handleRequestLineage()}
        />
      </div>
      {error && <p className="error">{error}</p>}
      {runs.length === 0 ? (
          ...
          {runs.map((run) => (
            <li key={run.analysis_run_id} className="ticket-list-item">
              <AnalysisRunListButton
                caption={analysisRunCaption(run)}
                nextAction={analysisRunNextAction(run)}
                documentCountLabel={analysisRunDocumentCountLabel(run)}
                onOpen={() => void handleOpen(run.analysis_run_id)}
              />
            </li>
          ))}
        </ul>
