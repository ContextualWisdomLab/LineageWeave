...
describe("i18n", () => {
  ...
  it.each([
    [
      "ko",
      "잔여 지도가 IRT 주효과 이후 중심화 잔여의 설명되지 않은 몫 0.12을(를) 남깁니다. sales-lead 기준을 읽으려면 이 글을 여세요.",
    ],
    [
      "zh",
      "残差图在 IRT 主效应后留下中心化残差中未解释的份额 0.12。打开这篇帖子阅读 sales-lead。",
    ],
    [
      "ja",
      "残差マップはIRT主効果後の中心化残差のうち未説明の割合 0.12 を残します。この投稿を開いて sales-lead を読んでください。",
    ],
    [
      "vi",
      "Bản đồ phần dư để lại phần chưa giải thích 0.12 của phần dư đã căn giữa sau hiệu ứng chính IRT. Mở bài viết này để đọc sales-lead.",
    ],
  ] as const)("formats leftover-map unexplained share next action in %s", (locale, expected) => {
    setLocale(locale);
    expect(
      tf(
        "Leftover map leaves unexplained share {value} of centered leftover after IRT main effects. Open this post to read {criterion}.",
  ...
  it.each([
    [
      "ko",
      "IRT 주효과 이후 잔여 맵 랭크 1, 관측 Y 2.40, 기대 E 2.00, 설명되지 않은 몫 0.12를 읽은 다음, 이 글을 여세요.",
    ],
    [
      "zh",
      "阅读 IRT 主效应后的残余图秩 1、观测 Y 2.40、期望 E 2.00 与未解释份额 0.12，然后打开这篇帖子。",
    ],
    [
      "ja",
      "IRT主効果後の残差マップランク 1、観測 Y 2.40、期待 E 2.00、未説明の割合 0.12 を読んでから、この投稿を開いてください。",
    ],
    [
      "vi",
      "Đọc hạng bản đồ phần dư 1, Y quan sát 2.40, E kỳ vọng 2.00, và phần chưa giải thích 0.12 sau hiệu ứng chính IRT, rồi mở bài viết này.",
    ],
  ] as const)(
    "formats combined leftover evidence with unexplained share next action in %s",
    (locale, expected) => {
      setLocale(locale);
      expect(
        tf(
          "Read leftover map rank {rank}, observed Y {observed}, expected E {expected}, and unexplained share {value} after IRT main effects, then open this post.",
          { rank: "1", observed: "2.40", expected: "2.00", value: "0.12" },
        ),
      ).toBe(expected);
    },
  );
