...
describe("i18n", () => {
  ...
  it.each([
    ["ko", "잔여 지도 비교 그림 원점 (0.00, 0.00)"],
    ["zh", "残差地图比较图形原点 (0.00, 0.00)"],
    ["ja", "残差マップの比較図原点 (0.00, 0.00)"],
    ["vi", "gốc đồ họa so sánh bản đồ phần dư (0.00, 0.00)"],
  ] as const)(
    "formats leftover-map comparison graphic leftover-map origin in %s",
    (locale, expected) => {
      setLocale(locale);
      expect(
  ...
  it.each([
    ["ko", "잔차 축 원점 (0.00, 0.00)"],
    ["zh", "残差轴原点 (0.00, 0.00)"],
    ["ja", "残差軸原点 (0.00, 0.00)"],
    ["vi", "gốc trục phần dư (0.00, 0.00)"],
  ] as const)(
    "formats leftover-map leftover-axis leftover-map origin in %s",
    (locale, expected) => {
      setLocale(locale);
      expect(
        tf("leftover axis leftover-map origin {origin}", {
          origin: "(0.00, 0.00)",
        }),
      ).toBe(expected);
    },
  ...
  it.each([
    ["ko", "잔여 쌍 잔여 지도 원점 (0.00, 0.00)"],
    ["zh", "残差配对残差图原点 (0.00, 0.00)"],
    ["ja", "残差ペアの残差マップ原点 (0.00, 0.00)"],
    ["vi", "gốc cặp phần dư bản đồ phần dư (0.00, 0.00)"],
  ] as const)(
    "formats leftover-map pair leftover-map origin in %s",
    (locale, expected) => {
      setLocale(locale);
      expect(
        tf("leftover pair leftover-map origin {origin}", {
          origin: "(0.00, 0.00)",
        }),
      ).toBe(expected);
    },
