...
import { leftoverMapCompareListCriterionBadge, leftoverMapCompareListPostBadge, leftoverMapListCriterionBadge, leftoverMapListPostBadge } from "./leftoverMapCoordinates";
...
describe("leftoverMapPlotCriterionBadge", () => {
  ...
  it("names rank-0 leftover-map origin leftover-map item coordinates independently of leftover-map person coordinates", () => {
    expect(leftoverMapPlotCriterionBadge("sales-lead", 0, 0)).toEqual({
      key: LEFTOVER_MAP_PLOT_CRITERION_ORIGIN,
  ...
  it("stays distinct from leftover-map post ξ markers and leftover-map comparison graphic leftover-map criterion markers", () => {
    ...
    expect(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN).toBe(
      "leftover-map criterion {label} at leftover-map origin ζ {item}",
    );
    expect(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_PLOT_CRITERION);
    expect(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_CRITERION);
    expect(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN);
...
describe("leftoverMapComparePlotCriterionBadge", () => {
  ...
  it("names rank-0 leftover-map origin leftover-map item coordinates independently of leftover-map comparison graphic leftover-map post leftover-map origin leftover-map person coordinates", () => {
    expect(leftoverMapComparePlotCriterionBadge("sales-lead", 0, 0)).toEqual({
      key: LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN,
      values: { label: "sales-lead", item: "(0.00, 0.00)" },
    });
  ...
  it("stays distinct from leftover-map graphic leftover-map criterion markers and leftover-map post ξ markers", () => {
    ...
    expect(LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN).toBe(
      "leftover map comparison graphic leftover-map criterion {label} at leftover-map origin ζ {item}",
    );
    expect(LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_CRITERION);
    expect(LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN).not.toBe(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN);
    expect(LEFTOVER_MAP_COMPARE_PLOT_CRITERION).not.toBe(LEFTOVER_MAP_PLOT_CRITERION);
    expect(LEFTOVER_MAP_COMPARE_PLOT_CRITERION).not.toBe(LEFTOVER_MAP_PLOT_CRITERION_ORIGIN);
...
describe("leftoverMapPlotPostBadge", () => {
  ...
  it("names rank-0 leftover-map origin leftover-map person coordinates independently of leftover-map item coordinates", () => {
    ...
    expect(leftoverMapPlotPostBadge("Public post", 0, 0)?.key).not.toBe(
      LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN,
    );
    expect(leftoverMapPlotCriterionBadge("sales-lead", 0, 0)?.key).not.toBe(LEFTOVER_MAP_PLOT_POST_ACTION_ORIGIN);
  });
...
describe("leftoverMapComparePlotPostBadge", () => {
  ...
  it("names rank-0 leftover-map origin leftover-map person coordinates independently of leftover-map graphic leftover-map post leftover-map origin leftover-map person coordinates", () => {
    expect(leftoverMapComparePlotPostBadge("Public post", 0, 0)).toEqual({
      key: LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN,
      values: { title: "Public post", person: "(0.00, 0.00)" },
    });
  ...
  it("stays distinct from leftover-map graphic leftover-map post markers and leftover-map comparison graphic leftover-map criterion markers", () => {
    ...
    expect(LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN).toBe(
      "Open leftover map comparison graphic leftover-map post {title} at leftover-map origin ξ {person}",
    );
    expect(LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION);
    expect(LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_PLOT_POST_ACTION_ORIGIN);
    ...
    expect(LEFTOVER_MAP_COMPARE_PLOT_POST_ACTION_ORIGIN).not.toBe(LEFTOVER_MAP_COMPARE_PLOT_CRITERION_ORIGIN);
