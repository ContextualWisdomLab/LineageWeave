...
describe("AskAgentPanel public verification", () => {
  ...
  it("requires explicit consent and renders external evidence apart from cited posts", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          ...
          next_action: "Open the public evidence and review the internal claim.",
        }),
    ...
    expect(
      screen.getByRole("region", { name: "Public verification" }),
    ).toBeInTheDocument();
    expect(screen.getByText("Supported by public evidence")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "Public Apollo evidence" }),
