...
const EN: Record<ProjectHistoryMessageKey, string> = {
  ...
  summaryCounts: "{events} project posts · {connected} connected · lineage count {lineages} · {actors} actors in evidence",
...
const MESSAGES: Record<Locale, Record<ProjectHistoryMessageKey, string>> = {
  ...
  ko: {
    ...
    summaryCounts: "프로젝트 글 {events}개 · 연결된 글 {connected}개 · Lineage {lineages}개 · 근거 담당자 {actors}명",
  ...
  zh: {
    ...
    heading: "项目事件时间线",
    summaryCounts: "{events} 条项目记录 · {connected} 条已连接 · {lineages} 条 Lineage · 依据中出现 {actors} 名责任人",
  ...
  ja: {
    ...
    summaryCounts: "プロジェクト投稿 {events}件 · 接続済み {connected}件 · Lineage {lineages}件 · 根拠内の担当者 {actors}名",
  ...
  vi: {
    ...
    summaryCounts: "{events} bài dự án · {connected} bài đã kết nối · {lineages} lineage · {actors} người trong bằng chứng",
