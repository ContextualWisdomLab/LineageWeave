...
export function projectHistoryKeys(
  ...
  const sourceIdentity = sourceProjectCode?.trim() ? sourceProjectCode : sourceProjectName ?? "";
  const candidates = [...(evidence ?? []).map((project) => project.project_key), sourceIdentity];
...
const EN: Record<ProjectHistoryMessageKey, string> = {
  ...
  noVisibleEvents: "No project events are visible. Open a source record with project evidence or ask for access to the supporting records.",
...
const MESSAGES: Record<Locale, Record<ProjectHistoryMessageKey, string>> = {
  ...
  ko: {
    ...
    noVisibleEvents: "표시할 수 있는 프로젝트 이벤트가 없습니다. 프로젝트 근거가 있는 원천 기록을 열거나 뒷받침 기록의 접근 권한을 요청하세요.",
    eventDetail: "이벤트 상세",
  ...
  zh: {
    ...
    noVisibleEvents: "没有可见的项目事件。请打开包含项目依据的来源记录，或申请访问支持记录。",
    eventDetail: "事件详情",
    eventType: "显示事件类型",
    eventDate: "事件日期",
    timeBasisCode: "时间来源",
  ...
  ja: {
    ...
    noVisibleEvents: "表示できるプロジェクトイベントがありません。プロジェクト根拠を持つ原資料を開くか、根拠レコードへのアクセスを依頼してください。",
    eventDetail: "イベント詳細",
    eventType: "表示用イベント種別",
    eventDate: "イベント日",
    timeBasisCode: "時刻の出典",
  ...
  vi: {
    ...
    noVisibleEvents: "Không có sự kiện dự án nào khả kiến. Hãy mở bản ghi nguồn có bằng chứng dự án hoặc yêu cầu quyền truy cập vào bản ghi hỗ trợ.",
