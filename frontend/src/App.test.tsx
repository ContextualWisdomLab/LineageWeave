...
describe("App, authenticated", () => {
  ...
  it("shows the calibrated period-report mean theta on the home page", async () => {
    ...
    expect(screen.getByText("leftover map comparison leftover axis 2 origin tick 0.00 σ 0.86 18%")).toBeInTheDocument();
    expect(screen.queryByText("leftover map comparison leftover axis 1 tick 0.00 σ 1.84 82%")).not.toBeInTheDocument();
    expect(screen.queryByText("leftover map comparison leftover axis 2 tick 0.00 σ 0.86 18%")).not.toBeInTheDocument();
    ...
    expect(screen.getByText("leftover axis 1 origin tick 0.00 σ 1.84 82%")).toBeInTheDocument();
    expect(screen.getByText("leftover axis 2 origin tick 0.00 σ 0.86 18%")).toBeInTheDocument();
    expect(screen.queryByText("leftover axis 1 tick 0.00 σ 1.84 82%")).not.toBeInTheDocument();
    expect(screen.queryByText("leftover axis 2 tick 0.00 σ 0.86 18%")).not.toBeInTheDocument();
    ...
    const closestPair = screen.getByRole("button", {
      name: "leftover pair leftover-map post Public post at ξ (+0.50, +0.10)",
    });
    const farthestPair = screen.getByRole("button", {
      name: "leftover pair leftover-map post Specification revision requested at ξ (+0.90, +0.80)",
    });
  ...
  it("shows the grouping comparison strip and switches grouping on click", async () => {
    ...
    expect(
      within(screen.getByLabelText("Grouping comparison")).getByText(
        "leftover map comparison leftover axis 1 origin tick 0.00 σ 1.84 82%",
      ),
    ).toBeInTheDocument();
    expect(
      within(screen.getByLabelText("Grouping comparison")).getByText(
        "leftover map comparison leftover axis 2 origin tick 0.00 σ 0.86 18%",
      ),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      within(screen.getByLabelText("Grouping comparison")).queryByLabelText(
        "leftover-map axis 1 tick +0.50 σ 1.84 82%",
      ),
    ).not.toBeInTheDocument();
    expect(
      within(screen.getByLabelText("Grouping comparison")).getByText(
