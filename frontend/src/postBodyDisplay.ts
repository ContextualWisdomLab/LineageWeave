...
function normalizeMetricMarkup(raw: string): string {
  return raw.replace(
    METRIC_MARKUP,
    (_match, base: string, kind: string, leading: string, digits: string, trailing: string) => {
      const table = kind.toLowerCase() === "sup" ? SUPERSCRIPT_DIGITS : SUBSCRIPT_DIGITS;
      const converted = [...digits].map((digit) => table[Number(digit)]).join("");
      // Preserve whitespace (including newlines from pretty-printed source
      // HTML) around the digit rather than discarding it -- stripHtmlTags
      // relies on those newlines surviving to rejoin as spaces later.
      return `${base}${leading}${converted}${trailing}`;
    },
  );
}
...
function stripHtmlTags(text: string): string {
  const withScripts = normalizeScriptText(normalizeMetricMarkup(text));
