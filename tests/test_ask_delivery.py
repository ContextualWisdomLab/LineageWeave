...
GLOBAL_ASK_QUEUE_SOURCE_PATH = (
    Path(__file__).parents[1] / "backend" / "app" / "global_ask_queue.py"
)
...
def test_ask_delivery_uses_evidence_specific_owned_identifiers() -> None:
    ...
    ask_delivery_tree = ast.parse(ASK_DELIVERY_SOURCE_PATH.read_text(encoding="utf-8"))
    owned_identifiers = {
        syntax_node.id
    ...
    global_ask_queue_tree = ast.parse(
        GLOBAL_ASK_QUEUE_SOURCE_PATH.read_text(encoding="utf-8")
    )
    answer_function = next(
        syntax_node
        for syntax_node in global_ask_queue_tree.body
        if isinstance(syntax_node, ast.AsyncFunctionDef)
        and syntax_node.name == "compute_global_ask_answer"
    )
    answer_function_identifiers = {
        syntax_node.id
        for syntax_node in ast.walk(answer_function)
        if isinstance(syntax_node, ast.Name)
    }
    assert "delivery" not in answer_function_identifiers
    assert "ask_delivery" in answer_function_identifiers
...
def test_delivery_links_only_cited_evidence_without_keyword_classification() -> None:
    ...
    ask_delivery = build_ask_delivery(
        ...
        (
            {
                "post_id": "post/a",
                "facts": [{"kind": "source_field", "text": "Recorded"}],
            },
        ),
    )
...
def test_delivery_without_citations_cannot_offer_an_evidence_alert() -> None:
    ...
    assert ask_delivery["report"]["source_documents"] == []
    assert ask_delivery["alert"]["eligible"] is False
    assert ask_delivery["alert"]["watched_resource_uris"] == []
