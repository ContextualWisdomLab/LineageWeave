...
def test_leftover_pair_names_nullable_unexplained_share_column(schema_db) -> None:
    """Every install path preserves legacy pairs while naming unexplained leftover share."""
    with schema_db.cursor() as cur:
        cur.execute(
            """
            select column_name, is_nullable
    ...
    with schema_db.cursor() as cur:
        cur.execute(
            ...
            where conrelid = 'report_leftover_pair'::regclass
              and conname = 'leftover_pair_unexplained_share_nonnegative_chk'
            """
        )
