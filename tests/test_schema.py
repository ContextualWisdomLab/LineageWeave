...
_LEFTOVER_INNER_PRODUCT_MIGRATION = (
    Path(__file__).resolve().parents[1]
    / "migrations"
    / "0179_report_leftover_inner_product.sql"
)
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                ...
                cur.execute(_LEFTOVER_INNER_PRODUCT_MIGRATION.read_text())
            conn.commit()
            yield conn
        finally:
            conn.close()
...
def test_leftover_pair_names_nullable_inner_product_column(schema_db) -> None:
    """Every install path preserves legacy pairs while naming ξ·ζ."""
    with schema_db.cursor() as cur:
        cur.execute(
            """
    ...
    assert columns["leftover_inner_product"] == "YES"
