...
async def _rollback_noops_without_index_when_table_is_busy_scenario() -> None:
    database_name = f"lineageweave_ask_rollback_absent_{uuid.uuid4().hex[:12]}"
    database_dsn = _database_dsn(database_name)
    admin = await _connect_admin_or_skip()
    blocker: asyncpg.Connection | None = None
    observer: asyncpg.Connection | None = None
    ...
    try:
        ...
        assert (
            await observer.fetchval(
                "select to_regclass('public.global_ask_job_active_account_idx')"
            )
            is None
        )
    finally:
...
def test_rollback_without_target_is_idempotent_even_when_table_is_busy() -> None:
    """A rollback with no target index must not require an exclusive table lock."""
    asyncio.run(_rollback_noops_without_index_when_table_is_busy_scenario())
