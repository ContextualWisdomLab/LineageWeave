...
def test_translation_ledger_rollback_locks_resource_before_empty_guard() -> None:
    ...
    guard = "execute 'select exists (select 1 from ui_translation_resource)'"
...
def test_translation_ledger_rollback_tolerates_already_absent_resource_relation() -> None:
    ...
    assert "if resource_relation_exists then" in sql
    assert "execute 'select exists (select 1 from ui_translation_resource)'" in sql
