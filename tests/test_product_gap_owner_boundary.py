...
def test_latest_validation_overlay_pins_current_global_ask_candidate() -> None:
    ...
    assert "2026-09-24T05:20:00+09:00" in overlay
    assert "`2be09275a8843af3d62a838fa22d5cf091608707`" in overlay
    assert "migration and concurrency repairs" in overlay
...
def test_latest_validation_overlay_pins_similar_voc_retry_acceptance_boundary() -> None:
    ...
    assert "all 534 frontend tests" in overlay
    assert "`RetainedEvidenceRetry` was rendered" in overlay
    assert "320 x 568" in overlay
    assert "authenticated PostgreSQL/API acceptance is unavailable" in overlay
