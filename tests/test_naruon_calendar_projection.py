...
def test_client_sends_only_the_service_credential_and_parses_projection(monkeypatch) -> None:
    received = {}
    ...
    parsed_url = urlparse(received["url"])
...
def test_client_rejects_missing_or_control_bearing_service_tokens(token: str) -> None:
...
def test_parser_rejects_unknown_fields() -> None:
    with pytest.raises(NaruonCalendarContractError, match="unexpected"):
        parse_naruon_calendar_page(_page(_event(provider_url="https://provider.example")))
...
def test_json_schema_matches_the_parser_contract() -> None:
    import json
    ...
    assert occurrence_schema["properties"]["truth_status_code"]["const"] == "observed"
