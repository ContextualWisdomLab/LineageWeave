#!/usr/bin/env python3
"""Apply the one-shot Global Ask semantic candidate retrieval slice."""
...
CHANGELOG_ENTRY = dedent(
    ...
    - Global Ask candidate retrieval now searches persisted project mentions,
      role/responsibility/affiliation evidence, and cataloged Keyman mentions in
      addition to title, normalized body, and raw source hints. A buyer can ask
      with a semantic fact absent from source text and still receive the
      authorized cited post. Candidate discovery never bypasses post visibility
      or ABAC (ADR 0090 / ADR 0047 / ADR 0039).
...
ADR_TEXT = dedent(
    '''
    # ADR 0090: Global Ask candidates include persisted semantic evidence

    - Status: Accepted
    - Date: 2026-08-20
    - Owners: LineageWeave
    - Supersedes: the lexical-only implementation gap under ADR 0047
    ...
    affiliations, cataloged Keymen, title, and normalized body. The
    implementation loaded persisted semantic facts only after a post had
    already been selected from lexical text or raw source hints.

    A buyer could therefore see a project or owner in an evidence drawer, ask
    Global Ask with that exact semantic fact, and receive no cited post when the
    words were absent from raw source text. This was a source-contract mismatch,
    ...
    - Semantic-only buyer questions can discover their cited source post.
    - Global Ask and Board honor the same persisted evidence boundary.
...
RELEASE_TEXT = dedent(
    '''
    # 2.18.0 Global Ask finds semantic-only evidence

    Global Ask now finds an authorized post when a buyer term exists only in
    persisted project, responsibility, affiliation, or Keyman evidence. The
    cited post still crosses the same visibility and ABAC boundary before any
    content reaches contextual-orchestrator.

    This release does not claim exhaustive semantic search or invent a source,
...
FRAGMENT_TEXT = dedent(
    '''
    # 2.18.0 Global Ask semantic candidate retrieval

    Global Ask now nominates authorized source posts from persisted project,
    role/responsibility/affiliation, and Keyman evidence, not only lexical source
    text and raw source hints (ADR 0090).
    '''
)
...
def apply_implementation() -> None:
    ...
    if 'version = "2.18.0"' not in pyproject_text:
        replace_once("pyproject.toml", 'version = "2.17.0"', 'version = "2.18.0"')

    package = Path("frontend/package.json")
    package_text = package.read_text()
    if '"version": "2.18.0"' not in package_text:
        replace_once(
    ...
    if "## [2.18.0] - 2026-08-20" not in changelog_text:
    ...
    Path("docs/adr/0090-global-ask-semantic-candidate-retrieval.md").write_text(
        ADR_TEXT
    )
    Path("docs/releases").mkdir(parents=True, exist_ok=True)
    Path("docs/releases/2.18.0.md").write_text(RELEASE_TEXT)
    Path("CHANGELOG.d").mkdir(parents=True, exist_ok=True)
    Path("CHANGELOG.d/2.18.0.md").write_text(FRAGMENT_TEXT)
