...
replace = "  LEFTOVER_MAP_COMPARE_INCOMPLETE_POST_LABEL,\n"
if app.count(replace) != 1:
    raise SystemExit(f"App import anchor count={app.count(replace)}")
app = app.replace(replace, replace + "  LEFTOVER_MAP_COMPARE_INCOMPLETE_ITEM_LABEL,\n", 1)
replace = "            const comparisonIncompletePostCount = leftoverMapIncompletePostCount(row.leftover_map_coverage);\n"
if app.count(replace) != 1:
    raise SystemExit(f"App count anchor count={app.count(replace)}")
app = app.replace(replace, replace + "            const comparisonIncompleteItemCount = leftoverMapIncompleteItemCount(row.leftover_map_coverage);\n", 1)
replace = '''              {comparisonIncompletePostCount !== null ? (
...
'''
addition_render = '''              {comparisonIncompleteItemCount !== null ? (
...
'''
if app.count(replace) != 1:
    raise SystemExit(f"App render anchor count={app.count(replace)}")
app_path.write_text(app.replace(replace, replace + addition_render, 1))
...
# The previous r11 harness used bare `### Added`, which appears throughout released history.
# Bind the mutation to the unique Unreleased section instead.
