...
-- Existing installations predate the retired one-time-seed receipt. Canonical
-- constraint names are repository-owned, but definition substring matching is
-- not a semantic version: a malformed predecessor may mention ``retired`` and
-- still reject the required retired/null shape. Version the two canonical
-- constraints with catalog comments so any unversioned predecessor is replaced
-- once, while subsequent migration replay remains metadata-only. Only the two
-- known autogenerated predecessor names are otherwise migration-owned;
-- operator-defined integrity checks on ownership_state must survive replay.
do $customer_master_seed_ownership_retirement_contract$
declare
    stale_constraint text;
    state_constraint_oid oid;
    shape_constraint_oid oid;
    state_contract_marker constant text :=
        'lineageweave:customer-master-seed-ownership-state/v1';
    shape_contract_marker constant text :=
        'lineageweave:customer-master-seed-ownership-shape/v1';
begin
    select oid
      into state_constraint_oid
      from pg_constraint
     where conrelid = 'public.ui_translation_seed_ownership'::regclass
       and conname = 'ui_translation_seed_ownership_state_ck';

    if state_constraint_oid is not null
       and obj_description(state_constraint_oid, 'pg_constraint')
           is distinct from state_contract_marker then
        alter table public.ui_translation_seed_ownership
            drop constraint ui_translation_seed_ownership_state_ck;
    end if;

    select oid
      into shape_constraint_oid
      from pg_constraint
     where conrelid = 'public.ui_translation_seed_ownership'::regclass
       and conname = 'ui_translation_seed_ownership_shape_ck';

    if shape_constraint_oid is not null
       and obj_description(shape_constraint_oid, 'pg_constraint')
           is distinct from shape_contract_marker then
        alter table public.ui_translation_seed_ownership
    ...
    for stale_constraint in
        select conname
         ...
         where conrelid = 'public.ui_translation_seed_ownership'::regclass
           and contype = 'c'
           and conname in (
               'ui_translation_seed_ownership_ownership_state_check',
               'ui_translation_seed_ownership_check'
           )
    loop
        execute format(
    ...
    comment on constraint ui_translation_seed_ownership_state_ck
        on public.ui_translation_seed_ownership
        is 'lineageweave:customer-master-seed-ownership-state/v1';
    comment on constraint ui_translation_seed_ownership_shape_ck
        on public.ui_translation_seed_ownership
        is 'lineageweave:customer-master-seed-ownership-shape/v1';
end;
