...
-- Leftover interaction-map coordinates after IRT main effects (ADR 0121).
-- Complete-case Gabriel ξ / ζ on two leftover-map axes. Cascade with
-- the period score. A leftover-map post must be a member of this
-- report; a leftover-map criterion must be a CAT item on this report.
create table report_leftover_map_person (
    grouping_kind text not null,
    grouping_key text not null,
    period_code text not null,
    rubric_version text not null,
...
create index report_leftover_map_person_post_idx on report_leftover_map_person (post_id);

create table report_leftover_map_item (
    grouping_kind text not null,
    grouping_key text not null,
    period_code text not null,
    rubric_version text not null,
