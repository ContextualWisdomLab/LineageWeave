...
-- Leftover interaction map pairs after IRT main effects (ADR 0048 / 0168).
...
-- item on this report. Observed Y and expected E[Y|θ, item] persist so
-- leftover_residual is checkable as Y − E. Do not invent a leftover score.
create table report_leftover_pair (
    ...
    leftover_observed_score numeric not null,
    leftover_expected_score numeric not null,
    ...
    constraint leftover_pair_residual_identity
        check (abs(leftover_residual - (leftover_observed_score - leftover_expected_score)) < 1e-9)
);
