...
--
-- Registry evidence must be explicitly exported or deleted under an approved
-- retention procedure before the schema can be removed. The rollback is
-- idempotent only when every registry table is absent or empty.
...
drop function if exists lock_analysis_run_status_append();
