...
-- display name. Write-time persist binds only when exactly one catalog
-- row matches. Historical backfill copies a mention only when exactly
-- one mentioned person on that post has this role's actor_name.
