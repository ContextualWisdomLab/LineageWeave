...
> `ff7431bd1851c03e737808d22c6a2d43968582f9`; 38 PRs and 10 issues were
...
ADR 0264's candidate full O*NET 31.0 Content Model Reference (3,006 concepts),
...
| Construct granularity | ADR 0264's candidate branch publishes all 3,006 O*NET 31.0 Content Model Reference concepts with exact IDs, names, descriptions, and source-defined parents; it imports no ratings or person assertions | Pass exact-head review/checks and protected merge; import separately released occupation-element observations only through the provenance contract below |
...
| Job-family and job-series semantics | ADR 0263 adds a candidate authorized, immutable snapshot contract with separate source/node/hierarchy/binding tables, source-declared multiple membership and validity, explicit scheme-version-code occupation bindings, and no label-derived crosswalk; runtime rows remain unavailable | Pass exact-head review/checks and protected merge; prove identical replay and divergent-snapshot rejection against PostgreSQL, then add an authorized API/UI projection in a separate decision |
