...
A material subset is repaired. `scripts/smoke_test_oidc.py`, `scripts/k6_http_e2e.js` and `scripts/k6_mcp_e2e.js` use a separate confidential `lineageweave-test-automation` service-account client with `client_credentials`; Makefile and static contracts require an explicit `KEYCLOAK_CLIENT_SECRET` and keep machine evidence distinct from rendered browser login. The realm fixture contains that confidential client with exact API and MCP audience mappers, uses env-placeholder local fixture credentials, and requires S256 on the public frontend client. Compose and `.env.example` supply explicit synthetic local defaults for realm import. The public client intentionally still has direct grants enabled while the final repository consumers move.

Fresh fixture review found that Keycloak startup import was installed under the generic `realm-export.json` name instead of the realm-name startup-import convention. RED `ec51cec81b9c53ef16fc2535a444e9da2f1f6c31` pins the expected image destination; exact #1120 head `1ee02dbcd8a5d8e89082124a249f26f4b08e7274` minimally installs the existing source seed as `/opt/keycloak/data/import/lineageweave-demo-realm.json`. The source of truth remains `docker/keycloak/realm-export.json`; this repair does not create another identity owner.

The remaining RED is concrete: `backend/tests/test_api.py` still obtains its authenticated integration token through the public client's password grant, while `scripts/seed_demo_data.py` still uses password grants for Keycloak-admin bootstrap and for post-content warm-up. The machine service-account subject also still needs least-privilege representation in the seeded local authorization tables before k6 can exercise RBAC/ABAC rather than merely mint a valid machine token. Turning public direct grants off before these repairs would recreate the broken intermediate topology already rejected in review.

README #1117 remains stacked after the security migration because #1120 changes the exact `make smoke` and realm-import semantics README exposes. Latest ordinary two-parent/non-force convergence `4f82a98442bbc7fc64391b4dcb6d8e704f058e93` has prior README head `e0a3018a25d7d134ab8d9150e7b33d5dbe67b530` first and exact #1120 `1ee02dbcd...` second. Compare from #1120 has merge-base exactly `1ee02dbcd...`, `behind_by=0`, and `README.md` as the only effective child delta.

The former temporary self-removing realm writer is absent from the current tree; its predecessor run `35333568575` is historical. #1120 exact-head Tests `35343020528` and #1117 exact-head Tests `35343054860` are Draft-admission `skipped`, so none of the current source movement is hosted GREEN. Required order is: migrate backend/seed/bootstrap actors and seed the machine subject; disable public direct grants with S256 already present; prove focused/full exact-head GREEN; then prove rendered Authorization Code state/nonce/return-URL/product-session behavior. #1057 must consume that same fixture/token path in hosted CI rather than duplicate it.
