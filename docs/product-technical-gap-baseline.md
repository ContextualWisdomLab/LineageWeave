...
### Contextual-orchestrator boundary, local OIDC smoke and browser auth evidence
...
#1118 owns only the LineageWeave-local Keycloak token/JWKS/claim smoke operator contract. RED `53fe0ac34d8504bd48a0f90f73879135471370db` requires `make smoke` to activate the locked `dev` extra supplying PyJWT; `d6b5f67a642bf4892d92bd908dce35e99aaa4bbe` applies that Makefile repair. Follow-up RED `93b9e2169d393eda83751b7b9f0472ffb5007500` rejects stale direct-`python3` usage guidance. Fresh evidence-integrity RED `b3ec1c2b8dfac36fe0082e1e795878842696014e` rejects describing the Resource Owner Password Credentials/direct-access probe as a real browser OIDC round trip and requires transparent RFC 9700 classification. Exact head `04120daa95c709ed0b095e127e2fdbce055edc83` preserves the synthetic token request, live JWKS/signature/issuer/claim checks but labels the result only as a local compatibility smoke. RFC 9700 §2.4 says Resource Owner Password Credentials MUST NOT be used; RFC 10017 §7.3 requires browser OAuth/OIDC applications to use a redirect-based flow. Browser Authorization Code/PKCE, redirect/state/nonce, SSO, MFA and session behavior therefore require separate rendered evidence rather than extending this probe. Compare from #899 is `behind_by=0` and changes exactly `Makefile`, `scripts/smoke_test_oidc.py` and `tests/test_makefile_contract.py`. Current-head hosted acceptance is still absent because this lane remains Draft.
...
Issue #1119 now owns that buyer-visible LineageWeave consumer evidence gap. It must exercise the real product login control and redirect-based Authorization Code flow against the local product topology or an equivalent released Keyverse-compatible fixture, restore the return URL/session, call a real protected buyer API with the resulting browser session, and preserve fail-closed invalid-state/unauthorized behavior where LineageWeave owns those checks. PKCE/state/nonce assertions must follow the actual OIDC client contract rather than reimplementing Keyverse/Keycloak protocol logic. Identity/provider truth remains with Keyverse/Keycloak; #1119 owns only LineageWeave product E2E evidence.
...
| CO ownership / local OIDC smoke / package docs | #899 `a2da5875...` → #1118 `04120daa...` → #1117 `fca2641f...` | Owner boundary remains Proposed/Draft. The local smoke now declares its locked PyJWT environment and is explicitly limited to synthetic ROPC token/JWKS/claim compatibility; README is non-force stacked above that prerequisite and no longer overclaims browser OIDC evidence. | Promote parent-first with exact-head repository/security/static-analysis GREEN and qualifying approval; never treat the ROPC smoke as product-auth acceptance. |
| Browser Authorization Code acceptance | #1119 | Gap is now explicit; current App login test mocks `react-oidc-context`, while the local smoke bypasses the browser flow via direct-access grant. RFC 9700 §2.4 / RFC 10017 §7.3 rule out using that probe as browser acceptance. | Real rendered redirect-based Authorization Code buyer path with session/return-URL/protected-API evidence, applicable fail-closed state/permission cases, repository/security GREEN and qualifying approval. |
