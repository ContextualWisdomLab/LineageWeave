...
Fresh operator-contract review found a separate buyer-visible defect in the same #899 tree: `make smoke` invoked `uv run --locked` without an optional dependency set even though `scripts/smoke_test_oidc.py` imports `jwt` and PyJWT is optional rather than a base dependency. #1118 is the dedicated Draft sibling owner on exact #899. RED `53fe0ac34d8504bd48a0f90f73879135471370db` updates the existing Makefile contract to require the locked `dev` extra and prove that extra supplies PyJWT; `d6b5f67a642bf4892d92bd908dce35e99aaa4bbe` changes the smoke target to `uv run --locked --extra dev python scripts/smoke_test_oidc.py` without altering authentication semantics or canonical identity ownership.

Fresh review of that repair found the script still advertised direct `python3 scripts/smoke_test_oidc.py` checkout execution, which bypasses the dependency-declaring locked environment. Follow-up RED `93b9e2169d393eda83751b7b9f0472ffb5007500` rejects those stale instructions. Current exact head `6f3b1eded99a0cbf053cef944d2b292b6559c952` points canonical usage to `make smoke` and documents the equivalent locked `uv run --locked --extra dev ...` invocation; OIDC/JWT runtime semantics remain unchanged. Compare from #899 is `behind_by=0` and changes exactly `Makefile`, `scripts/smoke_test_oidc.py`, and `tests/test_makefile_contract.py`. Exact-head Tests `35304655655` are Draft-policy skipped, and there are no submitted reviews or inline review threads, so #1118 is not GREEN and must remain behind #899 until its parent and own evidence settle.
...
| CO ownership boundary / public package docs | #899 `a2da5875...` → #1117 `7accb384...` | #899 boundary repair remains Proposed/Draft; registry README child is non-force stacked and only README differs from the exact owner head. | Promote/integrate #899 first, then fresh #1117 exact-head checks and independent approval before public/package README integration. |
| Local OIDC smoke reproducibility | #899 `a2da5875...` → #1118 `6f3b1ede...` | Make target now activates the locked PyJWT-providing `dev` extra; follow-up RED also removes dependency-bypassing direct-`python3` usage guidance. Exact-head Tests `35304655655` are Draft-policy skipped. | Integrate #899 first, then executed exact-head repository/security/static-analysis GREEN and independent approval for #1118. |
