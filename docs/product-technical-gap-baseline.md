...
- **Zotero Integration**: (Resolved, LineageWeave side) `docs/adr/0029-zotero-local-reproducibility.md` documents the exact Local Zotero Connector/Web API workflow (`http://127.0.0.1:23119/api/`, save-then-verify via `/connector/saveItems` then `/api/users/0/items`) with a real reproducibility seed (a real DOI-linked paper). This is not just an aspiration: 10 modules in `lineageweave/`/`backend/` already carry real APA-style, year-parenthesized citations in their docstrings (e.g. `relation_verification.py` grounds its design in Thorne, Vlachos, Christodoulopoulos, & Mittal, 2018 (FEVER); `organization_name_resolution.py` in Miles & Bechhofer, 2009 (SKOS)) -- the practice is followed, not just described. What this repo cannot verify: whether papers/standards specifically referenced *by TEPP* (a separate repository) are synced the same way -- that is TEPP's own responsibility to audit, out of LineageWeave's scope.
