...
Issue #1119 owns the root grant/test-actor migration. Draft #1120 exact `44ab2f7202341b533e657714b8f97cd3e0021c8f` is its first executable security slice and is deliberately stacked on #1118 to preserve the active smoke-script writer boundary. Initial RED `d147db013...` parses the actual realm export and requires exactly one public `lineageweave-frontend` client with `standardFlowEnabled=true`, `directAccessGrantsEnabled=false`, and `serviceAccountsEnabled=false`. Follow-up RED `44ab2f720...` closes an additional standards gap: RFC 9700 §2.1.1 requires PKCE for public Authorization Code clients, and Keycloak leaves PKCE optional when the client PKCE method is blank, so the contract now requires `attributes["pkce.code.challenge.method"] == "S256"`. Parent source still has direct access enabled and no required PKCE method, so #1120 is intentionally RED. The causal realm edit has not been applied because the repository write surface rejected replacement of the same JSON file containing the synthetic demo credential; the tests were not weakened and the requirements were not hidden behind a build-time mutation.

Remaining #1119 work is unchanged: remove password-grant token acquisition from smoke/backend/k6/seed actors, classify machine/bootstrap identity explicitly, strengthen the existing rendered Authorization Code path with state/nonce/return-URL/product-session evidence, and preserve fail-closed tenant-scope behavior. #1057 separately owns making PostgreSQL + Keycloak + Valkey authenticated FastAPI bearer/JWKS/RBAC/ABAC execution mandatory in hosted CI and must consume #1119's standards-compliant fixture/token path rather than duplicate it. Identity/provider truth remains Keyverse/Keycloak-owned.
...
| ROPC removal + public-client PKCE + browser Authorization Code acceptance | #1119 → Draft #1120 `44ab2f720...` | Repository RED now enforces direct-access disablement, no service account, standard flow and mandatory PKCE S256 on the public client; parent fixture violates direct-access and PKCE requirements. | Apply causal realm fix without bypassing fixture/write safety; then remove password grants from smoke/backend/k6/seed, separate browser/machine/bootstrap semantics, add rendered Authorization Code state/nonce/return-URL/product-session evidence, security/static GREEN and approval. |
