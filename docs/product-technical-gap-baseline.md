...
Fresh operator-contract review found a separate buyer-visible defect in the same #899 tree: `make smoke` invoked `uv run --locked` without an optional dependency set even though `scripts/smoke_test_oidc.py` imports `jwt` and PyJWT is optional rather than a base dependency. #1118 is the dedicated Draft sibling owner on exact #899. RED `53fe0ac34d8504bd48a0f90f73879135471370db` updates the existing Makefile contract to require the locked `dev` extra and prove that extra supplies PyJWT; effective repair head `9669a18c41a8122d9adffe18ccd4a1587e72ed56` changes the smoke target to `uv run --locked --extra dev python scripts/smoke_test_oidc.py` without altering authentication semantics or canonical identity ownership. Compare from #899 is behind 0 and changes only `Makefile` plus `tests/test_makefile_contract.py`. Exact-head Tests `35300922032` are Draft-policy skipped, so #1118 is not GREEN and must remain behind #899 until its parent and own evidence settle.
...
| Local OIDC smoke reproducibility | #899 `a2da5875...` → #1118 `9669a18c...` | RED proves the smoke target omitted the optional PyJWT dependency; repair activates the locked `dev` extra. Exact-head Tests are Draft-policy skipped. | Integrate #899 first, then executed exact-head repository/security/static-analysis GREEN and independent approval for #1118. |
