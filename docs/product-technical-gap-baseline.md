...
| #694 | `faadbae0` | preserves overlapping voice classifications and authorized aggregate filters; voice evidence and next actions remain visible when the surrounding Dashboard is pending or failed, concurrent loading uses one live region, and synthetic-only Demo identities cannot enter a real-source denominator. Focused backend/frontend, lint/build/Storybook, PostgreSQL constraint, and 1280×720 visual checks pass. It remains stacked on #692, so parent delivery, retargeting, fresh hosted checks, and independent review are required |
| #693 | `c747fcfa` | restores the missing review/delivery parent for durable semantic embedding and operations-worker changes accumulated after #688 merged into #640; current composition leaves token splitting with the orchestrator's Rust owner, reports event-loop progress instead of PID existence, requeues stale source-hash analysis, and pins the two-poll orchestrator cadence in regression. It remains stacked directly on #640 pending protected parent delivery |
