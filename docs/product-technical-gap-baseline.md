...
| Occupation-to-construct relations | No occupation rating is imported; ADR 0256 explicitly separates those observations from vocabulary linkages | Persist released source observations with source version, occupation code, element identifier, scale identifier, value, sample/error/CI/suppression metadata when supplied, and provenance; never invent or locally normalize a weight |
