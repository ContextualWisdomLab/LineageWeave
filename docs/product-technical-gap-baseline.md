...
| Product consumption | ADR 0258 defines a candidate authenticated occupation-rating API; ADR 0259 adds a candidate Dashboard evidence view with exact source selection, sample/error context, text warnings, artifact links, responsive overflow, and distinct unavailable/empty states. Component/API tests and Storybook build pass; synthetic populated scenes were visually audited at 1440×900 and 390×844, including the mobile horizontal-scroll next action. Protected delivery and authenticated runtime evidence remain absent | Pass exact-head review/checks and protected merge; verify the authenticated API and rendered Dashboard against an authorized imported source using only aggregate/non-identifying evidence, and add a source catalog selector after an authoritative catalog read contract exists |
