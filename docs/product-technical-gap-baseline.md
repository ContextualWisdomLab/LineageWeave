...
Issue #1119 owns the root grant/test-actor migration. Draft #1120 is now at intermediate exact `630fae8d6da0bc371a18833311e63cd1a0d1e288`, directly on #1118. Initial RED `d147db013...` parses the actual realm export and requires exactly one public `lineageweave-frontend` client with `standardFlowEnabled=true`, `directAccessGrantsEnabled=false`, and `serviceAccountsEnabled=false`. Follow-up RED `44ab2f720...` also requires `attributes["pkce.code.challenge.method"] == "S256"` because RFC 9700 §2.1.1 requires PKCE for public Authorization Code clients and Keycloak otherwise leaves the client PKCE requirement optional.

The source realm fixture still violates both REDs at this intermediate head. Direct Contents-API replacement is rejected by the repository connector safety boundary because the same synthetic JSON fixture also carries demo credentials; the contract was not weakened, unrelated credential data was not rewritten, and no build-time security mutation was introduced. Exact `630fae8d...` adds only a purpose-bounded self-removing workflow transport around that write restriction. Push run `35333568575` is the sole current-head causal realm-repair execution: it is ancestry-guarded to `44ab2f720...`, changes only the public-client direct-grant and PKCE settings, executes `tests/test_oidc_security_contract.py`, verifies the JSON is the only source delta, removes itself, and creates a normal forward commit. It remains queued before runner assignment. Canonical queue owner `.github#712` records this specimen in comment `5728561626`; the current run must not be cancelled or replaced with a wake/no-op commit, runner-label workaround or Draft/Ready churn. Ordinary PR Tests `35333572654` are Draft-policy skipped. Until the one-shot self-removes and lands its source commit, #1120 remains RED and no product/security GREEN is claimed.

Fresh protected-source inventory still finds `grant_type=password` in the smoke script, backend authenticated test fixture, both k6 buyer paths and seed/bootstrap script. The generic form-encoding assertion in `tests/test_http_client.py` is not itself an auth actor and must not be mechanically deleted. Remaining #1119 work is therefore unchanged: remove password-grant token acquisition from real browser/public-client actors, classify machine/bootstrap identity explicitly, migrate privileged bootstrap to a standards-compliant mechanism, strengthen the existing rendered Authorization Code path with state/nonce/return-URL/product-session evidence, and preserve fail-closed tenant-scope behavior. #1057 separately owns making PostgreSQL + Keycloak + Valkey authenticated FastAPI bearer/JWKS/RBAC/ABAC execution mandatory in hosted CI and must consume #1119's standards-compliant fixture/token path rather than duplicate it. Identity/provider truth remains Keyverse/Keycloak-owned.
...
| ROPC removal + public-client PKCE + browser Authorization Code acceptance | #1119 → Draft #1120 intermediate `630fae8d...` | RED enforces public-client direct-grant disablement and mandatory PKCE S256. Source still violates both; self-removing causal-repair run `35333568575` is queued pre-runner and owned by `.github#712` evidence `5728561626`. | Let the sole current repair execute/self-remove, verify exact source delta and focused GREEN; then remove remaining password grants from smoke/backend/k6/seed, separate browser/machine/bootstrap semantics, add rendered Authorization Code state/nonce/return-URL/product-session evidence, security/static GREEN and approval. |
