...
| ---: | --- | --- |
| #691 | `ed4c46fb` | failed measurement and topic-lineage runs offer a retry action that creates and starts a new current-snapshot request while preserving terminal history; the latest baseline parent is integrated without force, while exact-head hosted checks and new review findings remain non-terminal |
...
| #667 | `92e3b2e1` | baseline refresh plus repeated-turn conversation-key repair, stale-history response fencing, uniquely numbered conversation/legacy-ontology ADRs, source-lock protection for every gathered Ask source, protected ontology publication proof, five-locale analysis-run guidance, and reproducible canonical/legacy publication URL statuses; reopening a repeated question now retains every later turn, focused frontend/backend/documentation checks pass, and exact-head hosted checks plus independent approval remain pending |
