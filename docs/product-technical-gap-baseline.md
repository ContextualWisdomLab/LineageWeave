...
Machine smoke and backend auth now consume one shared fail-closed RS256/JWK selector. RED `c069efdbdd7e2452d9b13a79812dbf061b2201c1`, dependency-safe refinement `060ba8a60ad9adb210be9881286e80a30b5b0cd2`, and repair through `896e911e9afce185c2637acc7f67c92389d8a35e` close the earlier verifier split without moving provider discovery/cache/issuer/audience ownership.

Fresh review then found a separate machine-identity reproducibility gap. LineageWeave authorization resolves exact OIDC `sub` into normalized local authorization, but the confidential automation client previously had no deterministic service-account subject in the disposable realm fixture. RED `8747af046407fb31e4627ed8220ea406418a4209` requires a credential-free `service-account-lineageweave-test-automation` user with exact subject `33333333-3333-4333-8333-333333333333`; source repair `2e94a4d7fa2c6042cd30b3ef3ddca4a5b91fa3d4` adds only that fixture identity. Exact-head Tests `35358628816` were Draft-policy skipped, so this is source repair, not hosted acceptance.

The deterministic machine `sub` does not itself grant product authorization. It is now a stable key that can be inserted into `user_account`, `account_affiliation` and `account_role_assignment` with least privilege. Until that normalized state is seeded, machine k6 proves token acquisition rather than the complete RBAC/ABAC buyer boundary.

The remaining ROPC RED is concrete. `backend/tests/test_api.py` still uses two distinct public-client password-grant principals; collapsing those subjects would invalidate its cross-account authorization proof. `scripts/seed_demo_data.py` still has a password-grant `admin-cli` lookup and a separate password-grant post-content warm-up. The first is unnecessary subject rediscovery because the human IDs are already deterministic; the second must migrate to a standards-compliant product actor without inheriting bootstrap privilege. Public direct grants therefore remain enabled until these consumers move.

README #1117 follows the security migration and was immediately ordinary/non-force reconverged after #1120 moved. Exact child head is `5b88860346a77545f2d4651b2b2c626441757fad`; compare from #1120 exact `2e94a4d...` has merge-base exactly at that parent, `behind_by=0`, and `README.md` as the only effective child delta. No predecessor validation receipt transfers.
...
| ROPC removal + public-client PKCE + browser Authorization Code acceptance | #1119 → #1120 `2e94a4d...` → README #1117 `5b888603...` | Machine actors use a confidential client; S256, deterministic human subjects, deterministic machine service-account subject and shared JWT/JWK verification exist. Overall actor RED remains open; exact-head Tests are skipped. | Remove admin subject rediscovery; migrate backend/warm-up actors; seed least-privilege normalized machine/product authorization; disable public direct grants; focused/full GREEN; rendered browser evidence; security/static GREEN and approval. |
