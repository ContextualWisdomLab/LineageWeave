...
## Current review observation — 2026-09-07 00:54 KST

Protected `main` is `83eba56149eb802cd63642c507c324c9976ec78e`.
The repair implementation is `ce8270118bbc6c2b57a3f6c26063eafec342970e`
on PR #964; this documentation commit does not change its executable files.
The [dated exact-ref inventory](development-loop-20260907-review.json)
separates implementation, pre-repair Checks, review heads, and runtime census.
Requests were sequential observations, not an atomic GitHub snapshot. Re-fetch
all heads and rules before a merge. Older sections below are historical only.

### Authority and ownership

Read LineageWeave's current `docs/product-requirements.md` before selecting
work, together with contextual-orchestrator's remote `docs/product_planning.md`
and `docs/architecture.md`. ADR 0213 retains the provider-before-pool boundary;
ADR 0246 retains all twelve atomic Voices without inventing combination codes.
Current main's ADR 0251 is the FJA I/O-psychology decision, not the combination
contract: Voice combination candidates use ADR 0256 and primary history uses
ADR 0252. This identity distinction must survive PRD/ADR convergence; no
unrelated decision is silently reassigned or overridden here.

GitHub's repository API freshly confirms `ContextualWisdomLab/LineageWeave`,
`RankWeave`, `ThreadWeave`, `TEPP`, `contextual-orchestrator`, and lowercase
`ContextualWisdomLab/disksage`. The PRD's `DiskSage` spelling is stale. No
DiskSage integration is introduced. No new research or numeric policy is
claimed. The [k6 fail contract](https://grafana.com/docs/k6/latest/javascript-api/k6/fail/)
establishes that error messages reach stderr, supporting status-only diagnostic
validation. DeepWiki has no indexed LineageWeave entry and Context7 returned a
quota error; neither is represented as verified documentation evidence.

### Review repair and protected delivery

The inventory covers 124 open PRs, including six Ready PRs at collection time.
All six Ready heads (#961/#929/#914/#911/#802/#780) and Voice children
#934/#935/#936/#937 had zero unresolved threads, with complete thread pages;
none had a current-head independent approval in the returned review window.
The earlier #964 repair workflow failed at its source-replacement assertion
before validation or product promotion. Its proposed code was inspected, then
the repair was applied and tested directly against the unchanged product head.

PR #964 now rejects null/array/scalar MCP envelopes, missing result members,
invalid tool results and structured content, and absent response bodies before
field access. The diagnostic contains the operation and HTTP status, never
response content. Thirteen new executable cases failed before the repair;
all 18 JavaScript cases and both Python harness contracts pass afterward.
...
job after a terminal verdict. They are not passing checks and are not evidence
of a product defect. No central gate was weakened or blindly rerun.
The three in-progress runs observed belong to open #961 at its current head;
none was cancelled. No protected merge or merge SHA is claimed.

### Greatest remaining customer gap and integration constraints

The largest unclosed acceptance gap remains authorized evidence continuity:
carrying Post versus derivation evidence, hidden-evidence omission, truth and
cutoff history, and paged multi-Voice JSON-LD preservation across API and UI.
This is a product-contract priority, not a numeric ranking or inferred user
study. Implementation ownership remains #780, #934/#935 (paged export),
...
0301 (#902/#838), 0305 (#844/#843), 0335 (#877/#876), and 0355 (#920/#915).
No conflicting newly added forward-migration identity was found; paired
rollback files are not duplicate forward migrations. SQL compatibility is
still unverified. Ten open candidates touch `frontend/src/api.ts` and thirteen
touch `backend/app/main.py`; the inventory records their owners. Previously
identified release-title conflicts remain unresolved, not verified package
collisions. PR #961 owns the current 2.28.0 runtime identity repair. This
change allocates no ADR, API, schema, or release identity.

### Current non-identifying evidence and unavailable load acceptance

A fresh read-only census of formal `lineageweave` PostgreSQL observed 43,189
Posts and 130 Ask jobs (105 succeeded, 25 failed). These are operational totals,
not a selected sample, population inference, semantic-quality result, or
candidate runtime validation. Formal services remain running. No mixed/private
source was used for load. Candidate synthetic authenticated k6 concurrency,
latency, errors, throughput, and correlated PostgreSQL/worker/Valkey/gateway
saturation remain unavailable; no performance change is justified by this
census. No temporary container was created, and no formal volume was removed.
Existing primary-checkout translation-ledger code, tests, and baseline edits
remain untouched.
