...
**Audited PR head:** #258 code commit `4228c48fd8795586ed6859ad4df047508a12f86f` for the synchronized ontology and customer-master hierarchy hardening.
**Active PR update:** Customer Master now has a cycle-safe, accessible hierarchy projection, and the ontology separates W3C ORG organization instances from SKOS classification concepts; exact-head frontend, Storybook, ontology, and hosted verification remain required before merge.
**Stacked product-gap update:** #302 preserves semantic document units across footnotes, nested lists, HTML/OOXML rows, and Markdown tables; its exact current head and hosted checks remain pending.
