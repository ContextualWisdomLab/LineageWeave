...
| Construct-to-work relations | ADR 0253's candidate branch imports all 1,417 official Ability/Essential Skill/Transferable Skill/Work Style linkages to Work Activities and Work Context with assertion-level source provenance and no invented weights | Pass exact-head review/checks and protected merge; keep these published relevance links distinct from occupation ratings and causal claims |
| Occupation-to-construct relations | No occupation rating is imported; ADR 0253 explicitly separates those observations from vocabulary linkages | Persist released source observations with source version, occupation code, element identifier, scale identifier, value, sample/error/CI/suppression metadata when supplied, and provenance; never invent or locally normalize a weight |
...
|---:|---|---|---|
| #732 | `b2dd06f4` | `feat/onet-31-content-model-ontology` | unstable; full tests, frontend, and Devin Review queued; CodeRabbit passed |
