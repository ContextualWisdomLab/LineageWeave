...
| Occupation-to-construct relations | ADR 0254 defines a candidate 3NF, release/source-partitioned observation store preserving value, optional category, sample/error/CI, suppression, relevance, update date, source digest, and domain provenance; no corpus is imported | Pass exact-head review/checks and protected merge; add a deterministic source importer that validates declared scale bounds and source dates before loading pinned public artifacts; never invent or locally normalize a weight |
