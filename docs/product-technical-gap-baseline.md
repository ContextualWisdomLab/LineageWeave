...
| ---: | --- | --- |
| #691 | `74975def` | failed measurement-run retry candidate stacked on the baseline branch; exact-head checks and independent review are still non-terminal, and the stack conflict state must be re-evaluated after its parent moves |
