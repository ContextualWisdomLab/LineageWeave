...
#1118 exact `04120daa95c709ed0b095e127e2fdbce055edc83` owns only the local smoke dependency/evidence-semantics repair. Causal grant migration is issue #1119 / Draft #1120 exact `896e911e9afce185c2637acc7f67c92389d8a35e`; identity/provider truth stays Keyverse/Keycloak-owned and #1057 separately owns hosted PostgreSQL + Keycloak + Valkey execution.

#1120's executable RED still requires the public `lineageweave-frontend` client to use standard flow with direct access/service accounts disabled, mandatory S256 PKCE, and repository-owned executable auth actors to stop consuming `grant_type=password`. A material subset is repaired: smoke plus HTTP/MCP k6 use a separate confidential `lineageweave-test-automation` service-account client with `client_credentials`; operator contracts require an explicit `KEYCLOAK_CLIENT_SECRET`; the realm fixture contains API/MCP audience mappers, S256, deterministic synthetic human subject IDs and environment placeholders. Keycloak startup import installs the existing realm source as `/opt/keycloak/data/import/lineageweave-demo-realm.json` while `docker/keycloak/realm-export.json` remains the source of truth.

ADR 0028 keeps the production Keyverse decision Accepted while marking the local #1120 hardening amendment Proposed. The amendment makes the actor boundary explicit: rendered browser Authorization Code + PKCE, machine `client_credentials`, distinct product-test subjects, normalized local authorization, and purpose-bound bootstrap. It also records that the two synthetic human `sub` values are deterministic in the repository-owned realm fixture, so `scripts/seed_demo_data.py` should consume that fixture truth rather than authenticate to Keycloak Admin REST solely to rediscover those same IDs. A privileged bootstrap client is justified only if an actual Admin REST operation remains after that simplification.

Fresh current-head review found an additional evidence-integrity defect inside the migrated machine path. `scripts/smoke_test_oidc.py` carried a second, weaker JWT/JWKS key selector than `backend/app/auth.py`: the smoke accepted a missing JWT `kid` by selecting the first JWKS entry and omitted the product verifier's RSA, RS256, signing-use and `verify` key-operation filters. That allowed operator smoke evidence to accept a token/JWK shape the product boundary would reject. RED `c069efdbdd7e2452d9b13a79812dbf061b2201c1`, dependency-safe refinement `060ba8a60ad9adb210be9881286e80a30b5b0cd2`, and the forward repair through exact `896e911e...` centralize fail-closed key selection in `lineageweave/oidc_jwks.py`. Backend auth translates shared failures to HTTP 401; smoke uses the same selector without pulling the backend optional-dependency set into the operator command. `tests/test_oidc_jwks.py` covers exact-kid selection, malformed/unsupported token headers, non-verification JWK shapes, invalid key sets and parser-error redaction. Exact-head Tests `35353135139` were skipped by Draft admission, so this source repair has no hosted execution receipt yet.
...
README #1117 follows the security migration. When #1120 moved to `896e911e...`, #1117 was immediately ordinary/non-force reconverged to exact `e25b10f4ce0f5dea1b44a1dc17557537b6af60df`, with prior #1117 `1260cd535...` first parent and exact #1120 second parent. Fresh compare from #1120 has merge-base exactly `896e911e...`, `behind_by=0`, and `README.md` as the only effective child delta. Exact-head Tests `35353186825` were Draft-policy skipped; no predecessor validation receipt transfers to either moved head.
