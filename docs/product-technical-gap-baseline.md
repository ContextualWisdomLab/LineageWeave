...
| Official worker-function vocabulary (post-snapshot observation, 2026-08-26 22:21 KST) | Protected `main` has no addressable DOT/FJA Data/People/Things vocabulary. PR #709 exact head `2fc2f770` carries candidate ADR 0232 and adds the 24 complete official definitions, domains, and ordinal rank positions as SKOS concepts plus deterministic fail-closed reads. Its Fleishman/O*NET facets remain an explicitly qualitative editorial projection of published constructs, not an O*NET category assertion, fitted requirement, calibrated score, or authoritative term-level crosswalk | Land candidate ADR 0232 and its implementation together through normal protected gates, publish the deterministic ontology artifact from `main`, and keep every facet visibly qualitative unless a cited exact crosswalk and separate measurement decision establish stronger semantics |
