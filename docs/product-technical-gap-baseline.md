...
| #965 | Serve the current 2026-07-28 stateless MCP protocol without dropping legacy clients | ADR 0272 proposed; dual-era k6 lane and contract tests on this branch, not yet protected-main evidence |
...
| MCP current protocol | ADR 0218's executable acceptance hard-coded the 2025-11-25 handshake and the admission boundary rejected `Mcp-Method` / `Mcp-Name` preflights, though the pinned SDK serves 2026-07-28 from the same app. This branch adds ADR 0272, the dual-era k6 lane (112/112 modern, 64/64 legacy checks green on the synthetic stack), and fail-closed routing-header tests | Land through the protected gate, then retain representative infrastructure telemetry and an approved quota/SLO decision before any capacity claim |
