...
| ---: | --- | --- |
| #694 | `cd6fc637` | preserves overlapping voice classifications and authorized aggregate filters; the exact-head repair aligns the denominator with shared source eligibility and ABAC, restores the authenticated frontend transport, removes the raw aggregate from the public response, and replaces implementation-boundary labels with localized next-action copy. It remains stacked on #692, so parent delivery, retargeting to `main`, fresh hosted checks, and independent review are required |
