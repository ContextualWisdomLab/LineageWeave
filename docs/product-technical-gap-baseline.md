...
Current auth authority is #899 `a2da5875525cd0950999487ff8fe7d439284dbd2` → #1118 `04120daa95c709ed0b095e127e2fdbce055edc83` → #1120 `d2634858a888c98c3071393bdc851e4deb1ee6d2` → #1117 `9033347b244b48727f36d42c9bed5dcfd9684f78`.
...
Fresh RFC 7517 §§4.8–4.9 review found that predecessor `b87cdbdc...` ignored certificate thumbprint metadata entirely. A JWK could therefore advertise malformed, wrong-width, or contradictory `x5t`/`x5t#S256` values and still enter RS256 candidate counting; with the same `kid` this could manufacture false ambiguity. Test-first commit `e80e61793561ed205df3193acf8813290ed2f9dc` adds matching SHA-1/SHA-256 acceptance, malformed/noncanonical/wrong-width rejection, embedded-leaf mismatch rejection, a same-`kid` poisoning regression, and the RFC-permitted case where a syntactically valid thumbprint exists without an embedded `x5c` chain. Causal fix `d2634858a888c98c3071393bdc851e4deb1ee6d2` validates canonical unpadded Base64url digest width and, when `x5c` is locally available, compares the advertised thumbprint to the first certificate DER before candidate admission. It does not add `x5u` retrieval, external trust-anchor validation, provider topology, or Keyverse source ownership.

The thumbprint RED-head Tests `35462701032` and exact-fix-head Tests `35462733220` are terminal skipped under Draft admission, so this remains source/executable-contract repair rather than repository-wide GREEN. #1117 was immediately ordinary/non-force reconstructed from the exact new #1120 tree plus its existing README blob and advanced to `9033347b244b48727f36d42c9bed5dcfd9684f78`; fresh parent→child compare has merge-base exactly `d2634858...`, `behind_by=0`, and README-only effective delta. Child Tests `35462775072` is also Draft-policy skipped. No predecessor child workflow receipt transfers.

Remaining auth RED: `backend/tests/test_api.py` still mints analyst and admin tokens through public `lineageweave-frontend` Resource Owner Password Credentials; `scripts/seed_demo_data.py` still uses `admin-cli` password grant for human-subject lookup and a public-client analyst password grant for content warm-up. The realm already carries distinct confidential automation/admin service-account clients with deterministic subjects and DB authorization provisioning, so those existing actors are the migration target rather than a new identity mechanism. Public direct grants must stay enabled until those consumers move without collapsing authorization evidence. Metadata also still admits `pyjwt[crypto]>=2.8.0` while the lock resolves 2.13.0, and full rendered browser Authorization Code + PKCE/session evidence is absent.
...
| Authentication | #899 → #1118 → #1120 `d2634858...` → #1117 `9033347b...` | migration RED / JWKS source repair | migrate backend/seed password grants to the existing confidential service actors without collapsing viewer/admin semantics; then disable public direct grants, align PyJWT floor, and obtain hosted/browser proof; retain public-only, x5c, KeyUsage, and x5t/x5t#S256 invariants |
