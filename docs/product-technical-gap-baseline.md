...
| Authenticated read latency | ADR 0272 sets a product-owner-approved 20 ms maximum for every REST `GET` and MCP read, including authorization, database work, projection, and serialization. The Dashboard candidate now separates exact authorized summary/lifecycle rollups from a stable keyset page of evidence-rich cases, so continuation changes transport size without shrinking counts. On the 43,189-record runtime, a transaction-local custom plan reduced the direct default 20-Post page from a 42–64 ms database fetch to a 7.423 ms first route observation and 2.736–3.094 ms warm observations; single-Voice and visibility filters use the same exact maintained rollup instead of a full-window count. This is focused branch evidence, not canonical external acceptance. Search still measured 592–1,149 ms even with a custom plan, title order had a 20.96 ms first observation before readiness warming, Post metadata previously measured 26–59 ms cold, and the 1,898,576-byte body stream measured 16–19 ms diagnostic TTFB with 143–167 ms complete transfer. No exact-head cold/warm all-read inventory currently passes. | Replace search's full UUID materialization and giant disjunction with indexed candidate streams and keyset continuation, consolidate Post metadata into one authorized JSON statement, validate the new title/filter indexes and all-connection readiness warming, and measure default/50/filter/search/detail/body-TTFB externally after the single canonical rebuild. Keep this gap open until every inventoried read passes the maximum rather than an average or cache-only observation. |
