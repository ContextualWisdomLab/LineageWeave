...
Issue #1119 owns the root grant/test-actor migration. Draft #1120 exact `d147db0130431546cd5018dd2bb16ab5bbdc3a97` is its first executable security slice and is deliberately stacked on #1118 to preserve the active smoke-script writer boundary. RED `d147db013...` parses the actual realm export and requires exactly one public `lineageweave-frontend` client with `standardFlowEnabled=true`, `directAccessGrantsEnabled=false`, and `serviceAccountsEnabled=false`. Parent source still has `directAccessGrantsEnabled=true`, so #1120 is intentionally RED. The one-value causal realm edit was not applied because the repository write surface rejected replacement of the same JSON file containing the synthetic demo credential; the contract was not weakened or hidden behind a build-time mutation.

Remaining #1119 work is unchanged: remove password-grant token acquisition from smoke/backend/k6/seed actors, classify machine/bootstrap identity explicitly, strengthen the existing rendered Authorization Code path, and preserve fail-closed state/return-URL/tenant-scope behavior. #1057 separately owns making PostgreSQL + Keycloak + Valkey authenticated FastAPI bearer/JWKS/RBAC/ABAC execution mandatory in hosted CI and must consume #1119's standards-compliant fixture/token path rather than duplicate it. Identity/provider truth remains Keyverse/Keycloak-owned.
...
| ROPC removal + browser Authorization Code acceptance | #1119 → Draft #1120 `d147db013...` | First repository RED now enforces direct-access disablement on the public client; parent fixture still violates it. | Apply causal realm fix without bypassing fixture/write safety; then remove password grants from smoke/backend/k6/seed, separate browser/machine/bootstrap semantics, add rendered Authorization Code evidence, security/static GREEN and approval. |
