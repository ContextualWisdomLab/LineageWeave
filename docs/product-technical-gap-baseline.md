...
| #632 | `78f3f71e` | Global Ask provenance, public verification, knowledge cutoff, evidence-constrained query rewriting, shared ABAC/rewrite-failure review repair, #654 ontology-label readability, Semgrep static-SQL repair, and #655 authenticated durable MCP stack with live Streamable HTTP lifecycle and k6 evidence; checks and independent review remain required |
...
| Lineage arithmetic ownership | ADR 0225 assigns retrieval fusion and contribution arithmetic to RankWeave and measurement/estimation to TEPP and fast-mlsirm. RankWeave issue #45 owns the Rust production core. Exact RankWeave PR #41 head `dc287724` now contains a standalone PRD, absorbed ADR 0005 compatibility tests, and ADR 0006 for one Rust engine, a Python adapter without fallback arithmetic, provenance-bearing policies, deterministic CPU scheduling, and an explicit fail-closed GPU backend. Superseded RankWeave PR #42 closed after its exact head became an ancestor of #41. These documents and the current Python implementation are not a released engine | Land RankWeave #41 through its protected gate, implement and release the Rust envelope under issue #45, upgrade LineageWeave's immutable pin, add fail-closed consumer conformance, and then delete local scoring, renormalization, threshold, window, and residual-map arithmetic. Preserve candidate-level theoretical-minimum evidence separately from whole-channel unavailability; never invent or locally renormalize a policy |
