...
| Apple-Silicon mathematical acceleration | ADR 0226 macOS-native Rust owner service with authenticated MLX Metal execution receipts | The boundary applies to accepted TEPP and fast-mlsirm scientific Rust kernels; their owner implementations and actual Metal parity receipts remain required before activation. RankWeave's current dependency-free Python retrieval-fusion/evaluation contract is not a current MLX owner or Rust parity receipt. A future Rust vector-scoring owner remains separate migration evidence and requires an accepted contract before this accelerator boundary can apply. |
