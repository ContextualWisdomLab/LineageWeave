...
| Knowledge Graph prompt provenance | PR #632 maps every ontology-annotated graph fact to the visible post recorded in `knowledge_graph_edge_evidence`; earlier code collected all visible graph facts beneath the first source document, so a citation could imply that the wrong source supported the edge | Exact-head tests must prove post chat and Global Ask attach each fact only to its evidencing source, retain ABAC and prompt bounds, and merge through protected `main`; external verification remains a separate issue #272 contract |
