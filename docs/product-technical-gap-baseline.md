...
| ---: | --- | --- |
| #694 | `cd6fc637` | preserves overlapping voice classifications and authorized aggregate filters; the repaired stack aligns the denominator with shared source eligibility and ABAC, restores the authenticated frontend transport, removes raw internal response copy, aligns period semantics, and replaces implementation-boundary labels with localized next-action copy. Focused tests, Storybook build, and 1280×720 visual inspection pass. It remains stacked on #692, so parent delivery, retargeting, fresh hosted checks, and independent review are required |
