...
| #677 | `d9d14c0c` | ADR 0228 account-owned per-post Ask conversation history rebased onto the current baseline with localized titles, full conversation/transcript pagination, stale-response guards, repeated-turn preservation, and eligibility recheck; hosted checks requeued and independent review required |
| #667 | `81c0a0d2` | baseline refresh branch current head; BLOCKED with review required; stacked #669 and #671 are merged into this non-default branch |
...
2. Merge #618–#621 only after each exact head shows terminal
