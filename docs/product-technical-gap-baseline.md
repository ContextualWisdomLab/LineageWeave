...
| Voice semantic taxonomy | ADRs 0244/0246 and migrations 0230/0235 preserve the twelve-value source-post scheme separately from the six-value post-scoped organization relationship scheme, retain source/derived disagreement and multi-membership, and provide authorized overlap-aware aggregate filters. PR #640 candidate `9ad662d4` now constrains qualified Voice assignments to those twelve governed concepts, requires their existing truth and source provenance, validates product-relation subject kinds, and carries all twelve labels through the Dashboard and ontology publication. The candidate's full Python suite passed 2,301 tests with 16 environment skips; this is branch evidence, not protected-main or private-corpus accuracy evidence. | Land the stack, run bounded orchestrator backfill, and verify aggregate-only source/derived/disagreement/unavailable counts at one declared cutoff without exposing record identities |
...
| Authenticated read latency | ADR 0272 sets a product-owner-approved 20 ms maximum for every REST `GET` and MCP read, including authorization, database work, projection, and serialization. The Dashboard candidate now separates exact authorized summary/lifecycle rollups from a stable keyset page of evidence-rich cases, so continuation changes transport size without shrinking counts. On the 43,189-record runtime, a transaction-local custom plan reduced the direct default 20-Post page from a 42–64 ms database fetch to a 7.423 ms first route observation and 2.736–3.094 ms warm observations; single-Voice and visibility filters use the same exact maintained rollup instead of a full-window count. Post detail is now one prepared authorized statement, and Post search uses transaction-maintained body, source, ontology/evidence, and master-text projections rather than decompressing wide bodies or applying the removed 0.45 word-similarity guesses. PR #640 candidate `9ad662d4` also stops an unrelated locale write from refreshing all 43,188 authored projection rows: the measured no-op write took 2.240 ms and refreshed zero rows after the fix. Exact broad search still fails: a 1,761-match sample measured p95 47.337 ms and maximum 320.215 ms, while a wider one-character query exceeded 5 seconds; a candidate custom plan regressed and was removed. The 1,898,576-byte body stream measured 16–19 ms diagnostic TTFB with 143–167 ms complete transfer. No exact-head cold/warm all-read inventory currently passes. | Keep the exact-count broad-search gap open; optimize its authorized count/order access path without dropping evidence or substituting an average. Validate title/filter/detail/body-TTFB and every REST/MCP read externally after the canonical exact-head rebuild. |
