...
A second valid integrity RED `2b7951a0384add445b4c2086337397772dc008e5` covers PostgreSQL temporary-schema shadowing. The predecessor trigger functions referenced `ui_translation_seed_ownership` / `ui_translation_resource` without a fixed function search path, so a caller-created TEMP relation could override ownership lookup. The RED creates a public `blocked` reservation, a fake `pg_temp.ui_translation_seed_ownership` `owned` row, and requires rollback root deletion to remain rejected. Its hosted workflows were cancelled by the immediate successor and are not acceptance evidence. Causal repair `92cf139fac69729387e6e3dc744658b9355160d1` declares `SET search_path = pg_catalog, public, pg_temp` on all three ownership trigger functions, keeping permanent product/ownership relations ahead of caller TEMP relations without privilege elevation. `tests/test_customer_master_translation_seed_ownership.py` now covers this TEMP-shadow path in addition to ordering/provenance, ordinary operator edit/delete, byte-preserving seed/rollback refusal against unowned data, owned replay idempotency, owned rollback and empty-reservation cleanup. Both valid ownership threads remain open pending hosted exact-head GREEN.

#929 is Ready-for-validation only. Fresh exact-head Tests `35196538087`, PROV-O `35196538059`, Ontology Pages `35196538041`, SAST `35196537913`, Security `35196537979`, and CodeQL `35196537961` exist on `92cf139f...`. Tests jobs `105120998761` (Full suite/PostgreSQL) and `105120999182` (Frontend) are both pre-runner with `runner_id=null` and `steps=[]` at the latest snapshot. No queued/pending/skipped/predecessor receipt is GREEN and no qualifying current-head independent approval exists.

Direct consumer #932 has been ordinarily/non-force converged to `d03ce1c42b12e470607be4a7ca8023498db8fbdc` on parent #929 `92cf139f...` and remains Draft. Its two-parent convergence preserves the existing consumer tree while inheriting the TEMP-shadow regression and search-path repair through ancestry only; it does not copy ledger truth into frontend source. Product-wide locale admission remains the currently complete `ko/en/ja/zh/vi` set; the eight-locale Customer Master ledger is a separate publication contract. The publication thread remains unresolved because a complete and safely owned draft is not qualifying language/product review, immutable publication, hosted PostgreSQL GREEN, or authenticated browser evidence.
...
| Governed translation delivery | #929 `92cf139f...` -> #932 `d03ce1c4...` | 37×8 Customer Master draft remains unpublished; migration ownership preserves operator lifecycle; TEMP-shadow RED `2b7951a...` is repaired by fixed trigger search paths at `92cf139f...`; fresh Tests `35196538087` is pre-runner and #932 remains Draft. | Exact-head hosted PostgreSQL/full-suite GREEN, ownership/search-path thread resolution, independent language/product review, immutable publication, authenticated API/browser eight-locale evidence, state/responsive/keyboard/focus/screen-reader/CJK checks, Security/SAST/CodeQL and qualifying approval. |
