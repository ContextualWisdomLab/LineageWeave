...
Upgrade RED `c52e7bacf6bc5ef066fa4245e5fdcfa2b8905a6a` models an installation that already has predecessor ownership checks under the current canonical names but without `retired`. Repair `ca11b270211bc6a3d6d829f8f6dfaf9d0dc7a6fc` inspects those canonical definitions, replaces them only when `retired` is absent, verifies owned→retired deletion and keeps later replay idempotent.

Non-destructive-upgrade RED `db5435def512615902372a9fc95770bc8c2b3a2b` then proves that the predecessor stale-check loop was too broad: any non-canonical CHECK whose definition mentioned `ownership_state` could be dropped, including independently managed DBA/operator constraints. Exact repair `4b6d21c629b203bd3a7c36bbbbc943775b9a8f21` limits stale removal to the two known autogenerated predecessor names `ui_translation_seed_ownership_ownership_state_check` and `ui_translation_seed_ownership_check`. The real-PostgreSQL upgrade fixture requires `operator_ownership_state_nonempty_ck` to survive both the retirement-contract upgrade and replay, while the canonical predecessor checks still upgrade by definition.

#929 remains Ready-for-validation only. Fresh exact-head Tests `35232809986`, Security `35232810033`, Ontology Pages `35232809965`, PROV-O `35232809951`, SAST `35232810024`, and CodeQL `35232809958` exist on `4b6d21c...`. Tests jobs `105241028195` (Full suite/PostgreSQL) and `105241028440` (Frontend) are queued with `runner_id=null` and `steps=[]`. No queued/pending/skipped/predecessor receipt is GREEN and no qualifying current-head independent approval exists. The migration-ownership/replay thread remains open pending hosted exact-head GREEN.

Direct consumer #932 has been ordinarily/non-force two-parent converged to `5368b8917749d48ba7a6fab393ca070641f14f6d` on parent #929 `4b6d21c...` and remains Draft. The convergence preserves the existing consumer tree while inheriting review preservation, purpose-complete replay admission, seed retirement, constraint upgrade and operator-check preservation through ancestry only; it does not copy ledger truth into frontend source. Exact-head Tests `35232885744` is terminal `skipped` by Draft admission and is not GREEN. Product-wide locale admission remains the currently complete `ko/en/ja/zh/vi` set; the eight-locale Customer Master ledger is a separate publication contract. The publication thread remains unresolved because a complete, safely owned, non-resurrecting and non-destructive draft is not qualifying language/product review, immutable publication, hosted PostgreSQL GREEN, or authenticated browser evidence.
...
| Governed translation delivery | #929 `4b6d21c...` -> #932 `5368b891...` | 37×8 Customer Master draft remains unpublished. Ownership/TEMP-shadow/concurrency/review-preservation/retirement repairs remain present. Upgrade RED `c52e7bac...` / repair `ca11b270...` admits retired on same-named predecessor checks; non-destructive RED `db5435de...` / repair `4b6d21c...` preserves operator-owned CHECK constraints while replacing only known predecessor checks. #929 Tests `35232809986` has Full/PostgreSQL `105241028195` and Frontend `105241028440` queued pre-runner; #932 Tests `35232885744` is Draft-skipped. | Exact-head hosted PostgreSQL/full-suite GREEN, ownership/replay review resolution, fresh current-head review, independent language/product review, immutable publication, authenticated API/browser eight-locale evidence, state/responsive/keyboard/focus/screen-reader/CJK checks, Security/SAST/CodeQL and qualifying approval. |
