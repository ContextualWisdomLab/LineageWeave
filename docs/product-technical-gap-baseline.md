...
#899 exact `a2da5875525cd0950999487ff8fe7d439284dbd2` restores contextual-orchestrator source-of-truth ownership and keeps ADR 0300 `Proposed` while unmerged. Provider/model discovery, routing, fallback and provider credentials remain contextual-orchestrator concerns. Direct descendants #902 `9487e0299a148a10bfcf7b02110508078843d3ef`, #919 `d78390c56c9d9abcf4a97d2ffa5477704dcbb465`, #966 `f93715af6cbab2a39c22d7c170df2dc32bcd7456` and #1118 `04120daa95c709ed0b095e127e2fdbce055edc83` remain Draft; #915 remains below #902.

#1118 owns only the LineageWeave-local Keycloak token/JWKS/claim smoke operator contract. RED `53fe0ac34d8504bd48a0f90f73879135471370db` requires `make smoke` to activate the locked `dev` extra supplying PyJWT; `d6b5f67a642bf4892d92bd908dce35e99aaa4bbe` applies that Makefile repair. Follow-up RED `93b9e2169d393eda83751b7b9f0472ffb5007500` rejects stale direct-`python3` usage guidance. Fresh evidence-integrity RED `b3ec1c2b8dfac36fe0082e1e795878842696014e` rejects describing the Resource Owner Password Credentials/direct-access probe as a real browser OIDC round trip and requires transparent RFC 9700 classification. Exact head `04120daa95c709ed0b095e127e2fdbce055edc83` preserves the synthetic token request, live JWKS/signature/issuer/claim checks but labels the result only as a local compatibility smoke. Compare from #899 is `behind_by=0` and changes exactly `Makefile`, `scripts/smoke_test_oidc.py` and `tests/test_makefile_contract.py`. Current-head hosted acceptance is still absent because this lane remains Draft.

#1117 was immediately converged non-force after #1118 moved: ordinary two-parent anchor `873ae7b86cb487ada092b3cc69cbe1706726da08` preserves prior #1117 `6218b2be1a2df88484db02b6a95170765718472b` as first parent and exact #1118 `04120daa...` as second parent. Exact README head `fca2641f90ca0f2e331403848c05b2da7f31bb35` differs from #1118 only by `README.md`, preserves registry-safe canonical links, and no longer claims `make smoke` is browser OIDC acceptance. It records the same RFC 9700/RFC 10017 boundary and requires a separate rendered Authorization Code acceptance path.

Fresh protected-source inventory shows the root security gap is broader than evidence wording. `docker/keycloak/realm-export.json` enables `directAccessGrantsEnabled=true` on the actual public `lineageweave-frontend` browser client, and protected paths in `scripts/smoke_test_oidc.py`, `backend/tests/test_api.py`, `scripts/k6_http_e2e.js`, `scripts/k6_mcp_e2e.js`, and `scripts/seed_demo_data.py` use `grant_type=password`. RFC 9700 §2.4 says Resource Owner Password Credentials MUST NOT be used; RFC 10017 §7.3 requires browser OAuth/OIDC applications to use a redirect-based flow. Issue #1119 therefore owns the root LineageWeave test-topology migration: add a RED requiring the frontend client to disable direct access, remove password-grant token acquisition from browser/public-client test and buyer paths, replace privileged bootstrap with a standards-compliant mechanism, and add rendered Authorization Code acceptance. Browser versus machine/bootstrap fixtures must preserve their actual actor semantics; a machine token cannot be relabeled as end-user acceptance. Identity/provider truth remains with Keyverse/Keycloak and must not be copied into LineageWeave.
...
| CO ownership / local OIDC smoke / package docs | #899 `a2da5875...` → #1118 `04120daa...` → #1117 `fca2641f...` | Owner boundary remains Proposed/Draft. The local smoke now declares its locked PyJWT environment and is explicitly limited to synthetic ROPC token/JWKS/claim compatibility; README is non-force stacked above that prerequisite and no longer overclaims browser OIDC evidence. | Promote parent-first with exact-head repository/security/static-analysis GREEN and qualifying approval; #1118 evidence wording is not the root ROPC removal. |
| ROPC removal + browser Authorization Code acceptance | #1119 | Protected public frontend client still enables direct access; smoke/backend/k6/seed paths use password grants. Existing App login test mocks `react-oidc-context`; current smoke bypasses the browser flow. | RED requiring `directAccessGrantsEnabled=false`; standards-compliant browser/machine/bootstrap fixture separation; rendered Authorization Code session/return-URL/protected-API evidence; repository/security GREEN and qualifying approval. |
