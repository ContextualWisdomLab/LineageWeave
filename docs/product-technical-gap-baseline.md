...
#929 exact `d4f42f579663e88a9f9fd932332ddd47682da124b` is not authority; the translation owner remains #929 exact `d4f42f579663e88a9f9fd932332ddd47682da124b` only if live PR metadata still says so. The code-current owner relationship is #929 → #932 and must be re-read before promotion. Translation truth remains PostgreSQL-authoritative and separate from ontology labels; publication still requires independent language/product review, immutable publication, authenticated API/browser consumption and CJK/text-expansion/font-fallback evidence.
...
A fresh ADR/code-current review also found `docs/adr/0049-leftover-pair-report-ui.md` still normatively states the historical screen-reader names `Open leftover closest/farthest pair: {title}`. That text now contradicts the accepted visible-label-prefix product contract. Repair it only after the one-shot writer self-removes, so the same branch never has concurrent writers. The amended ADR must state that the localized rendered pair label is the accessible-name prefix, action/evidence context follows it, and non-finite evidence is omitted rather than announced as a placeholder.
...
| --- | --- | --- | --- |
| Customer Master governed translations | #929 → #932 | PostgreSQL-authoritative multilingual ledger lane exists; live exact heads/checks must be refreshed before promotion. | Hosted PostgreSQL/full-suite + security/static GREEN, independent language/product review, immutable publication, authenticated eight-locale browser acceptance. |
| Leftover-pair accessible action | #830 `bebd77c...` → #977 intermediate `cf3bc986...` | Product contract is repaired; inherited App integration selectors remain the isolated hosted RED. One-shot causal repair is queued pre-runner and ADR 0049 is now known stale. | Let the sole current-head writer execute/self-remove; verify exact App-only selector delta; repair ADR 0049; then fresh frontend/full-suite + Chromium GREEN, Security/CodeQL settlement, locale/font-fallback evidence and qualifying approval. |
