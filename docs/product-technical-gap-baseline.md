...
#1115 exact `6545b5ff7ed88d98daad74ca3ba8f8606dad3fc4` retains structural `HTMLParser` embedded-image extraction, the narrow data-URI allowlist, strict base64 validation, remote-image rejection and LF-correct provenance positioning. Tests `35065642195`, SAST `35065642221`, and Security `35065642147` are GREEN. CodeQL `35065642188` is now terminal **FAILURE**, but its three compatibility shards reached exact live PR/head/base verification and then failed only because the authenticated dispatch verdict remained `pending`; the later `Dispatch current-head CodeQL scan` job `104906720511` succeeded. Canonical producer `.github` repository-dispatch run `35152871013` is correctly bound to `LineageWeave#1115@6545b5ff.../main@83eba561.../35065642188`, but `validate-dispatch` job `104985222128` remains queued with `runner_id=0` and `steps=[]` on `.github/main@89b225db...`, so no `codeql-dispatch/*` terminal status exists on the consumer head. Exact owner-path evidence is `.github#1929` comment `5705660388`. This is central terminal-publication/runner evidence, not a new image-parser source regression, and no qualifying current-head approval exists.
...
| Embedded-image ingestion | #1115 `6545b5ff...` | Tests/SAST/Security GREEN; consumer CodeQL is terminal fail-closed on a pending authenticated verdict, and canonical producer `35152871013` is queued pre-runner. | Canonical producer scan/SARIF and terminal status publication, exact consumer compatibility GREEN, exact-head approval. |
...
| Report/comparison marker identity | #876 `ac19cbcc...` -> #1033 `a5538f5f...` -> #1034 `e2b6724b...` | ζ/ζ/ξ boundaries preserved; #1034 executable localized/fail-closed a11y coverage added; exact Tests remain queued pre-runner with owner evidence `5705642889`. | #1034 exact-head GREEN, remaining review-thread resolution, then parent-first rendered/security evidence and qualifying approval. |
