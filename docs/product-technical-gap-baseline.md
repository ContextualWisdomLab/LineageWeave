...
| Official worker-function vocabulary | Protected `main` has no addressable DOT/FJA Data/People/Things vocabulary. PR #709 exact head `91f342f4` adds the 24 official definitions, domains, and ordinal rank positions as SKOS concepts plus deterministic fail-closed reads. Its earlier editorial Fleishman/O*NET facet projection was removed because no cited authority supplied an exact term-level crosswalk | Merge #709 through normal protected gates, publish the deterministic ontology artifact from `main`, and require an authoritative exact crosswalk before adding any ability, skill, work-style, or behavioral mapping |
