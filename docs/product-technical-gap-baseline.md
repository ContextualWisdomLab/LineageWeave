...
One-shot run `34997230462` eventually received a GitHub-hosted runner. It verified the exact RED head successfully, then failed in `Apply minimal causal fix` because the automation literal for the import block contained indentation that the real source does not contain. GREEN, commit and push steps were skipped. This is an automation-code RCA, not a product verdict. The workflow was repaired at auxiliary exact `468571288ba8ef45065c188c8d3a28ff2105cf7e` using line-joined exact source blocks and the current checkout action; replacement run `35016883619` is still queued. Queued is not GREEN. Once #866 moves, every descendant must converge again from the resulting exact head.
