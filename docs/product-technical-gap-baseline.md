...
> Dashboard delivery snapshot: 2026-08-26 04:31 KST. Protected `main` was
...
> Audit snapshot: 2026-08-26 04:31 KST (refreshed by the autonomous merge
...
| ---: | --- | --- |
| #663 | `55bd4b04` (observed parent) | project ontology traversal plus cutoff/snapshot-frozen project focus and labels; this documentation commit advances the head after capture |
...
| #658 | `b681bd53` | evidence-honest Global Ask knowledge cutoff with client validation |
...
| #632 | `1025a942` | active stacked repair head; hosted checks and independent review required |
...
| Concurrent web responsiveness | ADR 0204 releases pooled transactions during provider work, and the synthetic Compose boundary has an authenticated k6 E2E harness for Ask enqueue, concurrent reads, and job polling. A 2026-08-26 non-exact 4-VU observation completed 87 iterations and 261/261 endpoint checks, with 1.53 s p95 combined reader latency; the prior observation had stalled `/api/posts` beyond 30 seconds. ADR 0212 combines two authorized filter-option queries into one round trip without narrowing ABAC-visible options. Neither older-image observation is exact-head evidence or a product guarantee, and no causal improvement is claimed | Rebuild an exact-head application image, run `make load-http` with declared environment concurrency/window, and retain raw distributions and resource configuration. Compare the post-list database plan and latency with ADR 0212 while preserving the complete authorized filter set; set no SLO until representative capacity evidence is approved |
