...
#983 is exact `4800c8180f44bb53feeff82c5d4d116e4c49ea3c` and owns frontend post-body parsing. The quote-aware scanner repair remains rooted at `63a72ef89f5a97aef1e0139430ee659efd8d9478`, but hosted predecessor Tests `35082024018` proved that repair was incomplete: the full PostgreSQL suite, frontend lint/build and Storybook were GREEN, while Vitest had one functional failure in `postBodyDisplay.quotedBoundaryCoverage.test.ts` and the unchanged 100% coverage gate also failed. The functional RED was `<p>First<br title="a > b">Second</p>` collapsing to one `First Second` segment. The scanner recognized the complete tag; the defect was semantic classification: `<br>` became one newline and `splitSemanticParagraphs()` intentionally joins single-newline soft wraps. Causal product repair `4800c8180f44bb53feeff82c5d4d116e4c49ea3c` maps structurally parsed `<br>` to `\n\n`, preserving the existing two-segment break contract without changing scanner, list/indent, image allowlist, remote-image, auth, or runtime policy. Preserved coverage artifact `frontend-coverage` id `10449646541`, digest `sha256:8b6e9a23629358930c24cd62191b6d10216d83a4d6cf5f38470c41d6513e4481`, measured lines 3300/3405 (96.91%), statements 3548/3729 (95.14%), functions 1008/1055 (95.54%), and branches 2743/3171 (86.50%). Earlier test-only commit `816423c46a4b27c42d1369d4d72c02d99eda5df6` covers the artifact-selected `i18n.ts` non-DOM `document` branch but does not close the repository-wide denominator. Current descendants are #984 `dc88ba2abf2c3381091abb1148a8c32bed426e40`, #992 `5d364361acc54f857dae01e20d212a14fbe6b2eb`, and sibling #985 `2ca71e2389a5e877a2231e51ff7ea37c840e8c31`, all ordinary/non-force and `behind_by=0` against their direct parent. Fresh exact-head Tests `35112680324`, SAST `35112680460`, Security `35112680339`, and CodeQL PR `35112680548` are queued; the live boundary review thread remains unresolved and predecessor receipts do not transfer.
...
| Post-body parser hardening | #983 `4800c818...` | Predecessor hosted run exposed one real quoted-attribute `<br>` semantic-boundary failure plus the 100% coverage RED. Causal source repair maps structurally parsed `<br>` to `\n\n`; artifact `10449646541` measures 96.91% lines / 95.14% statements / 95.54% functions / 86.50% branches. Descendants #984 `dc88ba2a...`, #992 `5d364361...`, #985 `2ca71e23...` are converged. Current required workflows are queued and the live review thread remains unresolved. | Fresh exact-head functional regression/full suite, 100% frontend coverage, rendered/security/CodeQL GREEN and qualifying approval; continue artifact-driven closure without denominator/gate weakening. |
