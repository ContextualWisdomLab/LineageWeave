# ADR 0282 — Name leftover-map item complete-case coverage on the graphic display
...
leftover-map complete-case coverage on the graphic display
...
post complete-case coverage on the leftover-map graphic display. The graphic
still has no item coverage caption, so a buyer who reads two criterion
diamonds can treat those markers as the scored-criterion census even after
the pair-list note names post coverage. Incomplete columns are dropped from
Gabriel factorization; hiding item coverage on the plot lets a sparse
...
item coverage from plotted criterion marker count, leftover-map distance, or
...
numbers. This protected increment uses **0282** so it does not collide with
...
On the leftover-map graphic display, caption persisted leftover-map item
complete-case coverage as `Leftover map used N of M scored criteria
(complete-case)`, using the persisted `map_item_count` / `scored_item_count`
integers. A missing coverage row, a non-integer count, a negative used count,
a non-positive scored count, or used greater than scored omits that leftover-map
item coverage caption and keeps the connecting lines and any leftover-map
...
observed, leftover expected, leftover-map rank, or leftover-map post coverage
caption. Coverage `0 of M` is shown when that persisted used count is a
non-negative integer and scored is a positive integer. Do not invent item
coverage from plotted criterion marker count, leftover-map distance,
leftover-map rank, leftover-map post coverage, or the count of unused axes.
Click a post marker to open that post. Criterion markers are not post buttons.
The grouping comparison strip (ADR 0149) stays on its reduced leftover payload
and does not gain this leftover-map item coverage caption. The pair-list note
(ADR 0168) stays post coverage.
...
complete-case coverage, and the plot names persisted leftover-map item
complete-case coverage; click a post marker or a pair button opens that post.
Hidden posts stay hidden. Rank-0 unused axes still plot at the origin and still
name item coverage when that coverage is persisted.
...
segments, and leftover-map complete-case coverage on the graphic display.
...
complete-case residual rectangle; item coverage names how many scored
criteria entered that factorization.)
