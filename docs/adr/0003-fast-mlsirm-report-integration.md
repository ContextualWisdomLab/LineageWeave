...
7. **Leftover-pair slice** (shipped in 0.71.2; ADR 0017 / 0018 / 0048 / 0049 / 0183): after
   IRT main effects, persist closest and farthest post–criterion pairs
   from the residual leftover map, and name unexplained leftover share
   `s = U_c² / R̃²` of centered leftover when Gabriel coordinates exist
   so the leftover cell the two-axis map does not reconstruct is not
   read as leftover residual or leftover-map distance. Do not persist
   leftover-map unexplained leftover `U` or two-axis reconstruction
