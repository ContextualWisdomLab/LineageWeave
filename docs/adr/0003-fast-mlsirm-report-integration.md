...
7. **Leftover-pair slice** (shipped in 0.71.2; ADR 0017 / 0018 / 0048 /
   0049, amended by ADR 0186): after IRT main effects, persist closest
   and farthest post–criterion pairs from the residual leftover map,
   including two-axis leftover-map reconstruction `R̂`. Do not fork
   LSIRM; do not invent a leftover-pair API inside `fast-mlsirm` in
   this slice.
