...
7. **Leftover-pair slice** (shipped in 0.71.2; ADR 0048 / 0049 / 0181): after
   IRT main effects, persist closest and farthest post–criterion pairs
   from the residual leftover map, and name two-axis leftover-map
   reconstruction `R̂ = ξ_{1:2} · ζ_{1:2}` when Gabriel coordinates
   exist so the reconstructed leftover cell is not read as leftover
   residual or leftover-map distance. Do not fork LSIRM; do not invent a
