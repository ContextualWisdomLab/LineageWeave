...
7. **Leftover-pair slice** (shipped in 0.71.2; ADR 0017 / 0018 / 0048 / 0049 / 0182): after
   ...
   `R̂`. Category probabilities and expected responses must use
   `fast-mlsirm`'s public Rust-backed prediction API (upstream PR #1279);
   LineageWeave must not reproduce GRM/GPCM parameter conventions locally.
   The residual map remains LineageWeave's product projection: do not
   fork LSIRM, and do not invent a leftover-pair API inside `fast-mlsirm`
   in this slice.
