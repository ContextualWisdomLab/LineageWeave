# ADR 0036 — Buyer chrome is 게시판, 고객 마스터, 달력, Ask Agent
...
buyer path. The buyer IA names four GNB destinations. Weekly and
monthly reports are scheduled newspapers published as board posts,
not GNB items and not click-to-export.

## Decision

1. Authorized buyer GNB is **게시판 / 고객 마스터 / 달력 / Ask Agent**.
   Cubee is not a product name. File names may keep `ask_cubee_*`.
...
5. Opening a regular post or a newspaper uses the same modules: 원문,
   ...
   Lineage, Ask Agent. Those post-detail modules do not become GNB
   items. Do not build a second calendar on the post.
6. 달력 is a traditional PIMS consume view. LineageWeave shows events
   from an independent CalDAV module. This repo does not plant a
   calendar kernel, CalDAV server, or event store. If the module is
   not wired, fail-close (`이 범위의 일정을 아직 받을 수 없습니다`).
   Do not invent events. Do not pull post 할 일 rows into that view.
7. Analysis-run list/start, tutor copy, TEPP receipts, leftover-pair
   sandbox, agent logs, and RankWeave as a home module stay off buyer
   chrome.
8. Every post is submitted on the existing Valkey activity stream so
...
9. TEPP time / multilevel / topic / KG is consumed from clues on the
...
10. Same Keyman, same win-pool, and same ontology object are join-key
...
11. Searxng hits appear only in Ask Agent / grounded Q&A as `미검증
    ...
    web.
12. Similar-topic scrape is a scheduler hook. Live Camoufox / article
...
13. Do not attach Figma from this Cloud VM. Tokens and Storybook
    frames live on the operator Remote machine. ADR 0002 still
    forbids pixel-matching. A missing Figma frame is not an extra
    invented screen.
14. Corp / PU are Keyverse attributes on the existing OIDC login, not
    a second login form.
15. Do not copy scraped articles or ontology promotions into mail or
    calendar.
...
switch to 고객 마스터, 달력, or Ask Agent. Empty slots name the next
human action.
