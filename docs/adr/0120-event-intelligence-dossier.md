# ADR 0120: Compose Event Intelligence without collapsing scientific authorities

- **Status:** Accepted
- **Date:** 2026-08-20
- **Decision owners:** LineageWeave product and scientific integration maintainers
- **Related:** ADR 0003, ADR 0004, ADR 0016, ADR 0034, ADR 0074, ADR 0079, TEPP ADR 0011
...
7. an optional contextual-orchestrator verdict that cites evidence and records
   ...
   the live pair-adjudication client uses orchestrator-owned auto routing with a
   strict JSON Schema, JSON-encodes candidate labels as untrusted evidence,
   requests the orchestration trace, and fails closed rather than
   regex-extracting a number from free-form text;
