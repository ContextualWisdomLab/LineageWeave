...
farthest from after main effects.”). Reconstruction next action:
leftover map reconstructs `R̂` after IRT main effects; open this
post to read the named criterion. A missing reconstruction keeps the
existing closest/farthest next action. Clicking the button opens that
post with the same handler as a member row. Reconstruction naming is
[ADR 0181](0181-leftover-map-reconstruction.md).
