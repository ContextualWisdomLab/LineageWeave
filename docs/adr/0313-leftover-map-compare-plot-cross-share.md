...
([ADR 0304](0304-leftover-map-compare-graphic.md)), leftover-map cross share
persistence ([ADR 0185](0185-leftover-map-cross-share.md)), leftover pairs on
...
reconstruction persistence ([ADR 0201](0201-leftover-map-reconstruction.md)), leftover-map unexplained leftover share persistence
([ADR 0233](0233-leftover-map-unexplained-share.md)), leftover-map explained leftover
share persistence ([ADR 0266](0266-leftover-map-explained-share.md)), and leftover-map coordinates
...
distance on pair segments ([ADR 0271](0271-leftover-map-segment-distance.md)), leftover-map
...
([ADR 0295](0295-leftover-map-compare-unexplained-share.md)), and leftover-map
...
ADR 0275 already captions period-report leftover-map pair segments with
`leftover-map cross share {label}` when that leftover-map cross share is
finite. ADR 0296 already captions grouping comparison leftover-pair
buttons with that same persisted leftover-map cross share under accessible name
`Leftover map comparison cross share`. ADR 0312 already captions
...
`leftover-map cross share {label}`, so a buyer who compares leftover
pairs can treat the period-report graphic caption as the comparison graphic
leftover-map cross share even after the strip names `x`. Hiding a distinct
comparison-graphic leftover-map cross share caption lets leftover-map
unexplained leftover share `s` or leftover residual `R` be read as leftover-map
cross share without a next action. When `R`, `R̂`, `U`, `x`, `s`,
and `e` are finite, `e + s + x = 1`; the comparison graphic must name the same
persisted `x` the pair row and strip already show.

This increment captions leftover-map cross share on the grouping
comparison leftover-map graphic from already-named leftover-map cross
share through formatLeftoverMapCrossShare. Comparison copy uses the accessible
name `leftover map comparison graphic cross share {label}` so it stays
distinct from hyphen `leftover-map cross share {label}` on the
period-report graphic and from strip `Leftover map comparison cross share`.
It does not add columns. It does not recompute leftover-map cross share
from `R̂`, `U`, and `R`, leftover-map reconstruction, leftover residual, leftover-map distance,
...
numbers. This protected increment uses **0313** so it does not collide with
leftover-map unexplained leftover share on the grouping comparison leftover-map graphic
...
(0304), leftover-map cross share on grouping comparison strip pair rows
(0296), leftover-map cross share on pair segments (0275), leftover-map
cross share persistence (0185), leftover-map graphic display (0268), leftover-map
coordinates (0267), or the dashboard stacks.

## Decision

On the grouping comparison leftover-map graphic, caption each pair segment
with persisted leftover-map cross share when formatLeftoverMapCrossShare
returns a usable badge, next to leftover-map unexplained leftover share `s`. Use the
distinct accessible name `leftover map comparison graphic cross share {label}`
so the graphic caption is not the strip badge (`Leftover map comparison cross share`)
and is not the period-report graphic caption (`leftover-map cross share {label}`).
A missing or non-finite `x` omits that leftover-map comparison graphic
cross share caption and keeps leftover-map unexplained leftover share `s` when
...
finite, leftover-map reconstruction `R̂` when
finite, leftover-map distance `d`, plus the strip leftover-map cross share badge.
Rank-0 origin cells still name `2R̂U/R² 0.00` when that persisted leftover-map
cross share is finite. A share greater than 1 is shown, never clamped. A finite
negative leftover-map cross share is shown, never clamped. Do not invent `x` from
`R̂`, `U`, and `R`, leftover-map reconstruction, leftover residual, leftover-map distance,
plotted coordinates, leftover-map rank, leftover-map axis share, leftover-map post
coverage, leftover-map item coverage, leftover-map incomplete post coverage,
leftover-map incomplete item coverage, leftover pair count, or the count of unused
axes. Click a post marker to open that post.

Leftover-map cross share omits independently of unexplained leftover share captions,
explained leftover share captions, reconstruction captions, coverage notes, and leftover-map distance. A missing leftover-map
cross share omits leftover-map comparison graphic cross share and keeps
a usable unexplained leftover share caption, a usable explained leftover share caption, a usable reconstruction caption, a usable
...
This increment does not caption leftover-map unexplained leftover on the comparison graphic
with a distinct comparison-graphic name. That leftover-map unexplained leftover already sits on
the strip through ADR 0297. A finite negative leftover on neighbouring fields is shown,
never clamped.
...
cross share on that graphic when formatLeftoverMapCrossShare
returns a usable badge. Rank-0 unused axes still plot at
the origin and still name `2R̂U/R² 0.00` when that persisted leftover-map
cross share is finite. Click a post marker or a pair button opens that post. Hidden posts
...
`R̂ = ξ_{1:2} · ζ_{1:2}`. Unexplained leftover is `U = R − R̂`. Cross share of
raw residual is `x = 2 R̂ U / R²`. Grouping comparison leftover-map
cross share captions that persisted share on the grouping comparison leftover-map
graphic only when formatLeftoverMapCrossShare returns a usable badge.
Rank-0 unused axes still name `2R̂U/R² 0.00` when that leftover-map
cross share is stored. A share greater than 1 is shown, never clamped. A finite
negative leftover-map cross share is shown, never clamped.)
