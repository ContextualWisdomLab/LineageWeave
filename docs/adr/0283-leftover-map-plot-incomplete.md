...
`report_leftover_map_coverage`. ADR 0281 already names persisted leftover-map
post complete-case coverage on the leftover-map graphic display. ADR 0282
already names persisted leftover-map item complete-case coverage on that
graphic. The graphic still has no incomplete-post caption, so a buyer who
reads `used N of M scored posts (complete-case)` can treat those N posts as
the full scored census even after incomplete rows were dropped. Incomplete
rows are dropped from Gabriel factorization; hiding dropped posts on the plot
lets a sparse post with one missing criterion vanish without a next action.

This increment names leftover-map incomplete post coverage on the
leftover-map graphic display. It does not add columns. It does not recompute
dropped posts from scored minus used, plotted marker count, leftover-map
distance, or leftover-map rank. It does not persist leftover-map inner
product, cosine, or length as separate columns. It does not land Post quality
on the leftover criterion. Leftover-map distance stays two-axis Euclidean. Do
...
numbers. This protected increment uses **0283** so it does not collide with
...
post coverage as `Leftover map dropped {dropped} incomplete posts`, using the
...
incomplete post caption and keeps the connecting lines and any leftover-map
...
observed, leftover expected, leftover-map rank, leftover-map post coverage, or
leftover-map item coverage caption. Dropped `0` is shown when that persisted
dropped count is a non-negative integer. Do not invent dropped posts from
scored minus used, plotted marker count, leftover-map distance, leftover-map
rank, leftover-map post coverage, leftover-map item coverage, or the count of
unused axes. Click a post marker to open that post. Criterion markers are not
post buttons. The grouping comparison strip (ADR 0149) stays on its reduced
leftover payload and does not gain this leftover-map incomplete post caption.
The pair-list note (ADR 0168) stays post coverage.
...
complete-case coverage, and the plot names persisted leftover-map incomplete
post coverage; click a post marker or a pair button opens that post.
Hidden posts stay hidden. Rank-0 unused axes still plot at the origin and still
name incomplete posts when that dropped count is persisted.
...
segments, leftover-map complete-case coverage on the graphic display, and
leftover-map item complete-case coverage on the graphic display.
...
complete-case residual rectangle; incomplete post coverage names how many
scored posts stayed out of that factorization.)
