# ADR 0301 — Name leftover-map rank on grouping comparison strip pair rows
...
([ADR 0280](0280-leftover-map-segment-rank.md)), leftover expected on grouping
...
fabricated interaction. ADR 0280 already names persisted leftover-map
rank on leftover-map graphic-display pair segments. ADR 0300 already
captions persisted leftover expected `E` on grouping comparison leftover-pair
buttons. The strip pair button still does not name leftover-map rank, so a
buyer who compares leftover pairs can treat leftover expected `E`, leftover
observed `Y`, leftover residual `R`, leftover-map unexplained leftover `U`,
leftover-map cross share `x`, leftover-map unexplained leftover share `s`,
leftover-map explained leftover share `e`, leftover-map reconstruction `R̂`,
or leftover-map distance `d` as a two-axis leftover map even after the pair
list and graphic already name `rank 0`. Hiding rank on the strip lets a
rank-0 origin cell with `d 0.00` and `E 0.00` be read as leftover structure
without a next action.

This increment captions persisted leftover-map rank on each grouping
comparison leftover-pair button through formatLeftoverMapRank. It does not
add columns. It does not recompute rank from plotted coordinates, leftover-map
distance, leftover expected, leftover observed, leftover residual, leftover-map
reconstruction, leftover-map explained leftover share, leftover-map unexplained
leftover share, leftover-map cross share, leftover-map unexplained leftover, or
the count of unused axes. It does not persist leftover-map inner product,
cosine, or length as separate columns. Do not invent a leftover score. Do not
invent a theta.

The dashboard stack already used neighbouring leftover facts under other
numbers. This protected increment uses **0301** so it does not collide with
leftover expected on grouping comparison strip pair rows (0300), leftover
observed on grouping comparison strip pair rows (0299), leftover residual
...
strip pair rows (0293), leftover-map rank on pair segments (0280), leftover expected on
pair segments (0279), leftover observed on pair segments (0278), leftover observed
`Y` / expected `E` (0163), leftover residual on pair segments (0277), leftover residual
disclosure (0162), leftover-map unexplained leftover on pair segments (0276), leftover-map
unexplained leftover persistence (0182), leftover-map reconstruction on pair segments
(0272), leftover-map reconstruction persistence (0201), leftover-map rank persistence
(0164), or the dashboard stacks.

## Decision

On the grouping comparison strip, caption each leftover pair button with
the same persisted leftover-map rank formatter as the pair-row `rank` badge,
next to leftover expected `E`, leftover observed `Y`, leftover residual `R`,
leftover-map unexplained leftover `U`, leftover-map cross share `x`, leftover-map
unexplained leftover share `s`, leftover-map explained leftover share `e`,
leftover-map reconstruction `R̂`, and leftover-map distance `d`. Use the distinct
accessible name `Leftover map comparison rank` so the strip badge is not the
graphic caption (`leftover-map rank {label}`). A missing rank, a negative rank,
or a non-integer rank omits that leftover-map comparison rank badge and keeps
leftover expected `E` when finite, leftover observed `Y` when finite, leftover
...
captions on the pair list and graphic. Rank-0 origin cells still name `rank 0`
when that persisted leftover-map rank is a non-negative integer. A finite negative
leftover on neighbouring badges is shown, never clamped. Do not invent rank from
plotted coordinates, leftover-map distance, leftover expected, leftover observed,
leftover residual, leftover-map reconstruction, leftover-map unexplained leftover,
leftover-map explained leftover share, leftover-map unexplained leftover share,
leftover-map cross share, leftover-map post coverage, leftover-map item coverage,
...
`Y` when finite, persisted leftover expected `E` when finite, and persisted
leftover-map rank when that rank is a non-negative integer.
...
leftover-map inner product, leftover-map cosine, leftover-map
...
leftover-map rank on pair segments, leftover-map unexplained leftover on pair
...
on the pair list, and leftover-map rank persistence.
...
`α_j − β_i`; typically `p = 2` for the interaction map. Leftover-map rank
is the number of Gabriel singular values above the leftover singular floor
after IRT main effects. Grouping comparison leftover-map rank names that
persisted rank on the strip pair row only when formatLeftoverMapRank returns
a usable badge. Rank-0 origin cells still name `rank 0`.)
