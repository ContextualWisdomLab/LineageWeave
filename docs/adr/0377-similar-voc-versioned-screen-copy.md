...
- The generic ownership migration itself has a bounded rollback. It must refuse while an exact `owned` Similar VOC receipt or another non-Customer-Master generic owner remains, but a `blocked`, `pending`, or `retired` receipt owns no resource and may be removed without deleting an operator-owned same-identity resource. The rollback must restore the pre-existing Customer Master trigger lane rather than dropping that owner boundary.
...
- Causal blocked-rollback repair `df9a39e98df366a6d49e3dbb8b635698e867cd2b` removes the resource-absence precondition and makes the ownership receipt the dependency boundary: unowned `pending|blocked|retired` receipts are removable, while an `owned` receipt still refuses generic-layer rollback.
