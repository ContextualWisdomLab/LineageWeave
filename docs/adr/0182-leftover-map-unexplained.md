...
As originally accepted, this increment did not persist leftover-map
reconstruction `R̂`. ADR 0201 now amends that storage decision so the
already-defined identity is directly auditable. This ADR still does
...
Reconstruction `R̂` is computed by the same two-axis path used for `U`;
ADR 0201 persists that value rather than discarding it.
