...
**Scope:** Migration 0013, ADR 0014, rollback, and real-PostgreSQL contract tests.

## Standards mapped to implementation

| Source | Product implication | Implemented evidence |
|---|---|---|
| W3C PROV-DM and PROV-O | Preserve identifiable entities, activities, agents, generation/use, and derivation without flattening provenance into display-only edges. | `analysis_source_snapshot`, `analysis_run`, authenticated requester, append-only status events, and immutable digests. |
...
| OpenAPI 3.2.0 | Define explicit versioned API schemas rather than exposing database rows. | API intentionally deferred; ADR 0014 requires authorization tests before a product surface is claimed. |
...
Public documentation may mention synthetic Demo Corp and aggregate ranges only.
...
| Idempotency is actor-scoped | Permit identical opaque keys for two accounts and reject reuse by the same account, including concurrent racers. |
