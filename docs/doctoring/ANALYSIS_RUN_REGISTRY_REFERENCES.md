...
| W3C Accessible Name and Description Computation 1.1 | Do not let `aria-label` replace visible text the operator must hear. | Analysis-run digest prefixes live in a labeled group; the prefixes remain the accessible contents of disclosure buttons. Home-list `aria-label` includes the next-action sentence when one is shown (pending and failed rows). |
| WCAG 2.2 Success Criterion 4.1.2 | Name, Role, Value: a control's name must include the information a sighted operator uses to choose the next action. | `analysisRunAccessibleName` appends the kind-specific next-action sentence when `analysisRunNextAction` is non-null. |
