...
the four GNB destinations below. Not a grant to put bot chrome,
leftover-pair sandbox, TEPP receipts, or a generate/export control on
#74.
**Date:** 2026-08-18

Enterprise buyer chrome is four GNB destinations. Weekly VOC is a
**filter on 게시판**. Weekly and monthly newspapers are **scheduled
board posts**, not GNB items and not click-to-export.

This spec names the buyer path from language already on
`feat/role-responsibility-agent-ontology`: board posts, Event Lineage,
Keymen, commitments, Orgmetra tenant grain, and Ask Agent grounded in
the ontology (ADR 0004).
...
| 달력 | Consume view for an independent CalDAV module. No calendar kernel in this repo. | Read published schedules | 이 범위의 일정을 아직 받을 수 없습니다 |
| Ask Agent | Source-grounded Q&A (`POST /api/ask-cubee`) | Ask what happened on a lineage | 이 사건 lineage에서 근거할 수 있는 질문이 아직 없습니다 |

Buyer-facing headings use the Korean labels except **Ask Agent**.
English names stay in file names, API fields, and ADR titles. File
names may keep `ask_cubee_*`. Cubee is not a product name.
...
- Opening any post (regular or newspaper) shows the same modules:
  ...
  lineage graph, Ask Agent. Pictures stay pictures. Missing 5W1H
  slots fail-closed (이 사건의 누가/언제가 아직 없습니다). Issue /
  commitment work stays this To Do module. Do not build a second
  calendar on the post. Opening 할 일 / 고객 약속 may deep-link to
  달력 later.
...
A 미검증 후보 promoted from Ask Agent lands here so the buyer can
...
Traditional PIMS destination. LineageWeave **shows** a calendar by
consuming an independent CalDAV module (Officeware repo). This repo
does not plant a calendar kernel, CalDAV server, or event store. If
the module is not wired, fail-close: 이 범위의 일정을 아직 받을 수
없습니다. Do not invent events. Do not pull post 할 일 rows into
this view as a fake calendar.
...
theta. Outside checks stay `미검증 후보` in Ask Agent / grounded
Q&A until attached on 고객 마스터. This Cloud slice does not hit
live Searxng. Not a tutor menu.
...
chrome, a mail outbox, a scrape console, a planted calendar kernel,
and any 생성 / 실행 / 내보내기 / 지금 만들기 control. Do not copy
scraped articles or ontology promotions into mail or calendar.
...
machine. A missing Figma frame is not an extra invented screen.
[ADR 0002](../adr/0002-figma-access-boundary.md).

## Implementation (v2.12.5)

The four GNB destinations are buyer chrome on `App.tsx` (ADR 0036).
...
internals, TEPP receipts, a calendar kernel, or θ / IRT / CAT / FIPC on
