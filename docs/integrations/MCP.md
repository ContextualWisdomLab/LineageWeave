...
`global_ask(question)` is read-only and idempotent with respect to LineageWeave
state. It returns:
...
IDs not present in the authorized source bundle are discarded. No Global Ask
row is written merely because an MCP client asked a question.

## Authentication and authorization

The MCP endpoint is an OAuth 2.1 protected resource:
...
2. `iss`, expiry, mandatory exact `kid`, and the configured MCP `audience` are
   verified;
3. optional `MCP_REQUIRED_SCOPES` are enforced by the MCP SDK;
4. the token `sub` must resolve to a provisioned `user_account`;
5. the account must have `post_read`;
6. every candidate and every lineage-expanded source is checked against the
...
`global_ask` is annotated read-only, so `writes` approval mode still prompts for
write-capable tools but does not turn this evidence query into a write.
...
client registration remains an authorization-server responsibility.
...
- contextual-orchestrator unavailable or malformed: tool error, no invented answer
- unknown citation ID: omitted from output

## Operational checks

A release must exercise the MCP SDK client against the in-process server, assert
tool annotations and structured output, verify unauthenticated HTTP rejection,
and run the same auth, ABAC, source-boundary, and citation regressions in the
normal test suite. `uv.lock` remains authoritative for the MCP SDK version.