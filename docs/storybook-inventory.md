...
| `Post/Recorded perspectives` | Read the imported primary and every evidence-connected additional Voice with its recorded truth state instead of flattening them into one compound category. `CombinedEvidence`, `CorrectedEvidence`, `RejectedEvidence`, and `NarrowViewport` cover the retained Observed state, corrected Proposed state, rejected evidence, and narrow layouts. The 2026-09-05 `voice-history-*-desktop-20260905.png` and `voice-history-*-mobile-20260905.png` screenshots in `docs/screenshots/` audit these existing tokens at 1440 and 390 CSS pixels; these are synthetic component renders, separate from authenticated API evidence. | `VoicePerspectiveList`, `ticket-list`, `post-meta` |
