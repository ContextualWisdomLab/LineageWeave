...
| `Customers/EntityTreeRow` | Read every visible customer entity exactly once; `PureCycle`, `SelfParent`, and `UnlistedParent` prove a cycled, self-parented, or unlisted parent renders as top level with an explicit ancestry note instead of disappearing. | `CustomerEntityTreeRow`, `customerEntityTree` |
