...
| `Customers/EntityTreeRow` | Read every visible customer entity exactly once; `PureCycle`, `SelfParent`, and `UnlistedParent` prove a cycled, self-parented, or unlisted parent renders as top level with an explicit ancestry note instead of disappearing, and `OrdinaryTree` proves an ordinary parent/child renders with no ancestry note. | `CustomerEntityTreeRow`, `customerEntityTree` |
