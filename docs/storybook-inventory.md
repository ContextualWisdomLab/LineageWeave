...
| `Ontology/OccupationRatingProfile` | Enter an exact O*NET-SOC code, inspect the published value beside its sample/error and warning, then open the rating or scale artifact. `InteractiveEvidenceReady`, `EvidenceReady`, `NarrowViewport`, `SourceUnavailable`, and `EmptyOccupation` cover the full form, populated table, horizontal mobile access, and honest absence states. | `OccupationRatingProfile`, native table, `--color-border`, `--size-control-min` |
