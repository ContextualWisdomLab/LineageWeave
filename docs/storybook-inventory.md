...
| `Post/Recorded perspectives` | Read the imported primary and every evidence-connected additional Voice without flattening them into one compound category. `CombinedEvidence` and `NarrowViewport` cover desktop and narrow layouts. | `VoicePerspectiveList`, `ticket-list`, `post-meta` |
