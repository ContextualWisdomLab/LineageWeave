...
| `Customers/EntityTreeRow` | Read every visible customer entity exactly once. `PureCycle`, `SelfParent`, and `UnlistedParent` prove malformed parentage stays visible with an explicit ancestry note; `OrdinaryTree` proves the normal parent/child case adds no warning. `LoadingRelatedPosts` keeps an expanded row in a named loading status without showing the empty message, while `EmptyRelatedPosts` proves a completed zero-result projection shows the empty state and no stale live status. | `CustomerEntityTreeRow`, `customerEntityTree` |
