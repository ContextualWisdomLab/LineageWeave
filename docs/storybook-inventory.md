...
| `Post/Similar VOC` | Compare ontology/semantic similar VOC and prior action evidence, then open the source. `WithActionHistory`, `Empty`, `Loading`, and `Unavailable` cover normal, empty, loading, and initial-query failure. `RetainedEvidenceRetry` covers a later-page failure with retained evidence. `EmptyNextPageRetry` covers a later-page failure after a page produced no displayable items: both retry stories request the failed next page instead of restarting page 1, while only the retained-evidence state claims that evidence remains visible. | `SimilarVocPanel.css`, `SimilarVocPanel`, `StatusNotice` |
