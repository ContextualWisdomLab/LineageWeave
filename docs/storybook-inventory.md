...
|---|---|---|
| `Evidence/PostBody` | `NumericExponentBoundary` keeps long numeric exponents literal beside supported quantity superscripts. `ImageRegionLocations` and `WhitespaceCaptionFallsBack` cover source image evidence and caption fallback. | `PostBody`, `postBodyDisplay` |
...
| `Workspace/OperationsDashboard` | Compare Event and post counts, inspect external-information coverage, then open the cited source behind a claim, handover, or repeat-issue fact. `EvidenceReady`, `NarrowViewport`, `AnalysisPendingAndMissingEvidence`, `AnalysisFailed`, `EvidenceWithoutProject`, and `LoadError` cover populated, mobile, evidence without a project association, unavailable-evidence, analysis-pending, retryable failure, and transport-error states. | `--color-dashboard-*`, `OperationsDashboard` |
