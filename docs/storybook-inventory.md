...
|---|---|---|
| `Workspace/OperationsDashboard` | Compare Event and post counts, inspect open/resolved/evidence-missing claim, rebid, and handover lifecycles, then open either observed milestone source. `EvidenceReady` proves exact endpoint duration plus open and missing states; `NarrowViewport`, `RequiredFactMissing`, `AnalysisPendingAndMissingEvidence`, `AnalysisFailed`, and `LoadError` cover mobile, explicit evidence-absence, analysis-pending, retryable failure, and transport-error states. | `--color-dashboard-*`, `OperationsDashboard` |
