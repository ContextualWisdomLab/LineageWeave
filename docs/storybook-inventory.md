...
| `Post/Occupational construct evidence` | Review the exact record words behind an O*NET construct, distinguish inference from source text, then open the official catalog definition. Ready, supported-empty, processing, unavailable, historical-cutoff unavailable, and narrow-viewport scenes preserve missing-vs-negative and live-vs-historical semantics. | Existing `post-evidence-list`, `post-badge`, `semantic-provenance`; `OccupationalConstructEvidence` |
