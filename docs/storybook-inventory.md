...
| `Post/SimilarVocPanel` | Compare ontology/semantic similar VOC and prior action evidence, then open the source; retry keeps saved evidence visible and names the next action without fabricating a score or weight. | `SimilarVocPanel.css`, `SimilarVocPanel`, `StatusNotice` |
