...
- Organization chips now show the other search-corroborated SKOS label when it
  is unique (`Demo Corp (DC)`). Affiliate-org, Keyman-affiliation, and
  counterparty-org chips reuse their existing accessible names with that
  caption. Related corporate nodes use the companion instead of the ontology
  class caption when one is present. A miss, pending row, identical labels, or
  a tie stays unlabeled (ADR 0119). After `make seed`, click Demo Corp (DC)
  and walk that catalog row.
