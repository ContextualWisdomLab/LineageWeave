    ...
    for pair in report.leftover_pairs:
        await conn.execute(
            ...
            insert into report_leftover_pair (
                ...
                leftover_map_person_length, leftover_map_item_length
            ...
            pair.leftover_map_person_length,
            pair.leftover_map_item_length,
        )
...
async def fetch_period_reports(
    ...
    leftover = await conn.fetch(  # nosemgrep: python.lang.security.audit.sqli.asyncpg-sqli.asyncpg-sqli
        ...
        select lp.grouping_key, lp.pair_kind, lp.post_id, lp.criterion_code,
               ...
               lp.leftover_map_person_length, lp.leftover_map_item_length,
               p.post_title,
               p.visibility_code, p.corporate_entity_id,
               ({_SOURCE_CONTEXT_PRESENT_SQL}) as has_real_source_context
                ...
                "leftover_pairs": [
                    {
                        ...
                        "leftover_map_person_length": (
                            None
                            if row["leftover_map_person_length"] is None
                            else float(row["leftover_map_person_length"])
                        ),
                        "leftover_map_item_length": (
                            None
                            if row["leftover_map_item_length"] is None
                            else float(row["leftover_map_item_length"])
                        ),
                        "visibility_code": row["visibility_code"],
                        "corporate_entity_id": str(row["corporate_entity_id"]),
                        "has_real_source_context": bool(row["has_real_source_context"]),
