...
async def load_occupational_construct_evidence_status(
    conn: Any, post_id: str, *, evidence_configured: bool = True
) -> str:
    ...
    if value in {"complete", "processing"}:
        return str(value)
    return "unavailable" if evidence_configured else "setup_required"
