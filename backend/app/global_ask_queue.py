...
from .config import GLOBAL_ASK_JOB_DEADLINE_SECONDS, load_settings
from .global_ask_history import (
...
async def enqueue_global_ask_job(
    ...
    *,
    ...
    pointing at nothing. ``conversation_id``/``anchor_post_id`` ride along
    on the durable row so the worker settling this job later still knows
    which conversation to append the turn to and which post the question
    was anchored on -- the caller must verify a non-null ``conversation_id``
    is one the requester actually owns before enqueueing.
    """
    job_id = await conn.fetchval(
        """
        insert into global_ask_job
            (requesting_account_id, question_text, conversation_id, anchor_post_id)
...
def _configured_tepp_client() -> TeppClient:
    """Build a fresh TEPP client from current settings for one job run."""
    settings = load_settings()
    return configured_tepp_client(
    ...
    *,
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            returning requesting_account_id, question_text, conversation_id, anchor_post_id
            """,
            job_id,
            RUNNING,
            QUEUED,
    ...
    try:
        ...
        anchor_post_id = (
            str(row["anchor_post_id"]) if row["anchor_post_id"] is not None else None
        )
        try:
            payload = await asyncio.wait_for(
                compute_global_ask_answer(
                    ...
                    anchor_post_id=anchor_post_id,
                    tepp_client=_configured_tepp_client(),
                ),
            ...
            # The provider/evidence-assembly boundary can raise exceptions
            # carrying provider response bodies, prompts, or other
            # forbidden content (issue #361) -- never let the raw message
            # reach the durable failure_detail a reader later polls for.
            raise RuntimeError(
                "Ask Agent is unavailable: contextual-orchestrator returned no complete evidence object"
            ) from exc

        def can_see(record: asyncpg.Record) -> bool:
            """Same public-or-affiliated rule retrieval already applied."""
            if record["visibility_code"] == "public":
                return True
            return str(record["corporate_entity_id"]) in entity_ids

        try:
            async with pool.acquire() as conn:
                persisted_conversation_id = await persist_turn(
                    conn,
                    str(row["requesting_account_id"]),
                    row["conversation_id"],
                    str(row["question_text"]),
                    payload["answer_text"],
                    payload.get("next_action"),
                    payload["source_post_ids"],
                    payload["cited_post_ids"],
                    payload["cited_post_evidence"],
                    can_see_post=can_see,
                )
            ...
            raise RuntimeError("conversation not found") from exc
        payload["conversation_id"] = str(persisted_conversation_id)
