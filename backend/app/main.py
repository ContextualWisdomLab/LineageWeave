"""FastAPI app: direct-PostgreSQL post list/detail endpoints, gated by
OIDC login (backend.app.auth) and RBAC + ABAC (this module).
...
ABAC gate, evaluated per row on top of the RBAC gate: a post is visible if
it is public, or if it is private and the requesting account is affiliated
with the post's owning corporate_entity_id. abac_policy's condition_expression
column is reserved for a future, richer per-policy DSL (documented in
migrations/0001_initial_schema.sql); Phase 1 implements exactly this one
fixed rule directly, since it is the only rule the product currently needs.
"""
