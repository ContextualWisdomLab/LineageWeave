...
class BoundedRequestBodyApp:
    """Reject ambiguous or oversized MCP POST bodies before downstream parsing.

    ``Content-Length`` is useful for an early rejection but is never trusted as
    the only resource boundary. Requests without it are read incrementally into
    a buffer capped at ``maximum_bytes``. A successful request is replayed once
    to the downstream ASGI application without changing its bytes.
    """
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        """Apply admission to HTTP POST and pass other ASGI traffic unchanged."""
...
def _header_values(scope: Scope, name: bytes) -> tuple[bytes, ...]:
    """Return every raw value for one case-insensitive ASGI request header."""
    return tuple(
        value
...
def _parse_content_length(
    ...
    *,
    ...
    if not decoded or not decoded.isdecimal():
        return _INVALID_LENGTH
    significant_digits = decoded.lstrip("0") or "0"
    maximum_digits = str(maximum_bytes)
    if len(significant_digits) > len(maximum_digits):
