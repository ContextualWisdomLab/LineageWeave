...
def test_fetch_period_reports_normalizes_nonfinite_cross_share(
    seeded_db,
) -> None:
    """A persisted non-finite leftover-map cross-share must not break report
    delivery: PostgreSQL numeric can hold NaN/Infinity, and strict JSON
    rejects them. The primary report read model must map them to null while
    preserving finite positive/zero/negative values.
    """
    import json
    ...
    try:
        with admin_conn.cursor() as cur:
            ...
            author_id, corp_id = cur.fetchone()
            _seed_demo_period_report(cur, author_id, corp_id, process_unit_id)
            # One known finite positive, one zero, one finite negative, one
            # SQL null, and the three non-finite numerics Postgres accepts.
            # The composite key allows only closest/farthest per grouping, so
            # spread the seven values across every seeded W02 leftover row.
            cur.execute(
                "select grouping_kind, grouping_key, post_id, criterion_code, pair_kind "
            ...
            for index, (kind, key, post_id, criterion_code, pair_kind) in enumerate(pair_rows):
                share = injected[index % len(injected)]
                cur.execute(
                    ...
                    (share, kind, key, pair_kind, post_id),
                )
    finally:
        admin_conn.close()
    ...
    async def _run():
        ...
        try:
            ...
            for kind in ("process_unit", "corporate_entity", "thread_group"):
                payloads.append(await fetch_period_reports(conn, kind, "2026-W02"))
            return payloads
        finally:
            await conn.close()
    ...
    # Strict JSON (allow_nan=False) is what the HTTP boundary renders with.
