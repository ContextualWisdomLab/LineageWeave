...
def test_seed_period_report_surfaces_on_get_reports(client, demo_analyst_token, seeded_db) -> None:
    ...
    assert all(
        pair.get("leftover_map_person_length") is None or pair["leftover_map_person_length"] >= 0
        for pair in high_report.get("leftover_pairs", [])
    )
    assert all(
        pair.get("leftover_map_item_length") is None or pair["leftover_map_item_length"] >= 0
        for pair in high_report.get("leftover_pairs", [])
    )
