...
Period leftover pairs (ADR 0048 / 0049 / 0119 / 0162 / 0163 / 0164 / 0183)
are computed in `lineageweave/leftover_pairs.py` from the residual after a
real GRM/GPCM score, never invented. Distances are Euclidean on the
...
unexplained leftover share `s = U_c² / R̃²` of centered leftover next
to leftover-map distance `d`. Centered leftover `U_c` and two-axis
reconstruction `R̂_c` are not persisted. They sit above the member
