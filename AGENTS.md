...
Period leftover pairs (ADR 0048 / 0049 / 0172) are computed in
...
persist to `report_leftover_pair` with leftover-map rank so rank 0
is not read as structure, and sit above the member list so a click
opens that post.
