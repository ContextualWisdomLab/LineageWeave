...
0164 / 0184) are computed in `lineageweave/leftover_pairs.py` from the
...
plus explained leftover share `e = R̂_c² / R̃²` of centered leftover
next to leftover-map distance `d`. Two-axis reconstruction `R̂_c` is
computed internally and is not persisted. They sit above the member
list so a click opens that post. Leftover-map axis share (ADR 0148) is
Gabriel inertia of residual SVD axes 1 and 2 and persists to
