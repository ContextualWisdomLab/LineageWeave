...
leftover structure. Unexplained leftover `U` and leftover-map cross
share `2R̂U/R̃²` sit next to leftover-map distance `d`. Leftover-map axis
share badges name Gabriel inertia of axes 1 and 2; open a leftover pair
to read the post–criterion cell. The shares do not invent a leftover
score. Opening Public post names the next action: read
