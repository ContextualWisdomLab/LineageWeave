...
sit above the member list with leftover-map reconstruction `R̂` and
unexplained leftover `U` next to leftover-map distance `d`, plus
leftover-map rank; rank 0 names no leftover structure. Leftover-map
axis share badges name Gabriel inertia of axes 1 and 2; open a
leftover pair to read the post–criterion cell.
