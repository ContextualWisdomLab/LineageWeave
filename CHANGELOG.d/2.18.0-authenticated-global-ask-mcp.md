...
- Added a separately deployable MCP 2025-06-18 Streamable HTTP resource server with one read-only `global_ask` tool.
...
- Added per-account bounded invocation rate limiting without logging question or answer text.
...
- Unknown browser Origins fail closed and JSON-RPC batching is rejected.
