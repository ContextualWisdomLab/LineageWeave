## 2.88.0 — Leftover-map axis share on leftover-map comparison graphic leftover-map axis ticks independently of leftover-map singular values
...
- Caption leftover-map comparison graphic leftover-map axis ticks with persisted leftover-map
  ...
  singular values (ADR 0331). After `make seed`, leftover-map comparison graphic leftover-map axis
  ticks name persisted leftover-map axis share when leftoverMapComparePlotTickAxisBadge
  ...
  `leftover map comparison graphic leftover-map axis {k} tick {value} σ {singular}`. Rank-0 unused
  axes still name leftover-map axis share `0%`. Leftover-map comparison graphic leftover-map axis ticks stay
