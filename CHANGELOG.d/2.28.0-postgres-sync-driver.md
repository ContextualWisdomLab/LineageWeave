...
- Preserve libpq `connect_timeout` semantics at the compatibility boundary:
  URI values remain decimal integers, zero or negative values mean no deadline,
  and explicit boolean, non-finite, or float-unrepresentable overrides fail
  through the adapter validation contract rather than leaking conversion errors.
