## 2.70.0 — Leftover-map cross share on grouping comparison leftover-map graphic

- Caption leftover-map cross share on the grouping comparison leftover-map
  graphic (ADR 0313). After `make seed`, closest and farthest leftover pairs
  ...
  cross share `x` on that grouping comparison leftover-map graphic; click a
  post marker or a pair button opens that post. A missing or non-finite `x`
  omits that leftover-map comparison graphic cross share caption and keeps
  ...
  cells still name `2R̂U/R² 0.00` when that persisted leftover-map cross share is
  finite. Comparison copy uses `leftover map comparison graphic cross
  ...
  with a distinct name. Do not invent `x` from `R̂`, `U`, and `R`, leftover-map reconstruction,
  ...
  never clamped. A finite negative leftover-map cross share is shown, never clamped.
