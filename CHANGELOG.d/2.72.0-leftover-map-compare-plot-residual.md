...
- Caption leftover residual on the grouping comparison leftover-map
  ...
  `R` on that grouping comparison leftover-map graphic; click a
  post marker or a pair button opens that post. A missing or non-finite `R`
  omits that leftover-map comparison graphic residual caption and keeps
  ...
  finite. Comparison copy uses `leftover map comparison graphic residual
  {label}` so it stays distinct from hyphen `leftover residual
