## 2.39.0 — Leftover-map item complete-case coverage on the graphic display

- Caption the leftover-map graphic display with persisted leftover-map
  item complete-case coverage (ADR 0282). After `make seed`, closest and
  ...
  list, and a plot caption that matches leftover-map item complete-case
  coverage; click a post marker or a pair button opens that post.
  A missing, non-integer, negative-used, non-positive-scored, or
  used-greater-than-scored item coverage omits that leftover-map item
  coverage caption. Coverage `0 of M` is shown when that persisted used
